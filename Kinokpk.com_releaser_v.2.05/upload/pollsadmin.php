...
if (!isset($_GET['action']))  {
    stdhead("Опросы");
   print('<table width="100%" border="1"><tr><td><a href="?action=add">Добавить опрос</a></td><td>Опросы v.2 by ZonD80</td></tr></table>');
   ...
   print('<tr><td><a href="polloverview.php?id='.$poll['id'].'">'.$poll['question'].'</a></td><td>'.get_date_time($poll['start']).'</td><td>'.(!is_null($poll['exp'])?(($poll['exp']< time())?get_date_time($poll['exp'])." (закрыт)":get_date_time($poll['exp'])):"Бесконечен")."</td><td><a href=\"?action=edit&id=".$poll['id']."\">E</a> / <a onClick=\"return confirm('Вы уверены?')\" href=\"?action=delete&id=".$poll['id']."\">D</a></td></tr>");
 }
 print("</table>");
...
elseif ($_GET['action'] == 'add') {
  ...
  <tr><td>Количество вариантов ответов(если опрос):</td><td><input type="text" name="howq" size="2"></td></tr><tr><td><input type="submit" value="Дальше"></td></tr></form></table>');
  stdfoot();
}
elseif ($_GET['action'] == 'add2') {
   ...
   print('<tr><td>Публичный?</td><td><input type="checkbox" name="public" value="yes"> (пользователи смогут видеть, кто и как голосовал)</td></tr>');
...
elseif ($_GET['action'] == 'edit') {
  ...
  print('<table width="100%" border="1"><form action="?action=saveedit&id='.$id.'" method="post"><tr><td>Вопрос:</td><td><input type="text" name="question" value="'.$pollres['question'].'"></td><tr><td>Истекает через:</td><td><input type="text" name="exp" value="'.(!is_null($pollres['exp'])?round(($pollres['exp']-time())/86400):"0").'" size="2"> дней 0 - бесконечно</td>');

   print('<tr><td>Публичный?</td><td><input type="checkbox" name="public" value="yes" '.(($pollres['public'] == 'yes')?"checked":"")."></td></tr>");
