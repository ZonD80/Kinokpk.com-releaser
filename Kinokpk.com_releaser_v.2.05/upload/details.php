...
//set new torrent as checked, by qwertzuiop
if(checkNewTorrent($id, $row["added"]))
{
  $coo_ch = $_COOKIE['torrents_vis'];
  setcookie("torrents_vis", ($coo_ch ? $coo_ch. "A" .$id : $id),0x7fffffff,"/");
...
$torrentid = (int) $_GET["id"];
				...
				if ($CURUSER["id"] != $row["owner"])  
                    tr("Пожаловаться", "<form method=\"post\" action=\"report.php\">&nbsp;<input name=motive cols=40 value=\"Причина\">&nbsp;<input type=\"submit\" value=\"Отправить\" /><input type=\"hidden\" name=\"torrentid\" value=\"$torrentid\"></form>", 1);
