...
<table border="1" cellspacing="0" cellpadding="5">
	...
	<tr>
		<td class="colhead" colspan="2"><?print $tracker_lang['upload_torrent'].(($descrtype == (1 or 2))?' [<a href="javascript:kinopoiskparser(\'parser\');">Заполнить форму, используя данные Kinopoisk.ru</a>]':'')?></td>
	</tr>
	<?
	...
	for ($i = 0; $i < $CACHEARRAY['max_images']; $i++) {
		$imagecontent.="<b>".$tracker_lang['image']." ".($i+1).":</b>&nbsp&nbsp<input type=file name=image$i size=80><br/><b>Или адрес картинки (URL):</b>&nbsp&nbsp<input type=\"text\" size=\"63\" name=\"img$i\"><hr/>";
	}
	...
	while ($dd = mysql_fetch_array($descrtarray)) {
		...
		elseif ($dd['input'] == 'option') {
			...
			$s .= "</select>\n";
			tr($dd['name'].(($dd['required'] == 'yes')?"<font color=\"red\">*</font>":"").($dd['spoiler']=='yes'?"<br/><small>Будет автоматически скрыто спойлером</small>":""),$s,1,1);
		}

		elseif ($dd['input'] == 'links') {
			tr($dd['name'].(($dd['required'] == 'yes')?"<font color=\"red\">*</font>":"").($dd['spoiler']=='yes'?"<br/><small>Будет автоматически скрыто спойлером</small>":""),"<textarea name=\"val[".$dd['id']."]\" rows=\"3\" cols=\"60\" wrap=\"soft\"></textarea><hr/>".$dd['description']."\n",1);
		}

		elseif ($dd['input'] == 'bbcode') {
			print("<tr><td class=rowhead style='padding: 3px'>".$dd['name'].(($dd['required'] == 'yes')?"<font color=\"red\">*</font>":"").($dd['spoiler']=='yes'?"<br/><small>Будет автоматически скрыто спойлером</small>":"")."</td><td>");
	...
	$s = "<table width=\"100%\"><tr><td colspan=\"2\">Выбрано тегов: <input type=\"text\" name=\"tagcount\" value=\"0\" readonly/></td></tr>";
