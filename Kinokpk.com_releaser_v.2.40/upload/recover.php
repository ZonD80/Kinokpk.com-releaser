...
EOD;
	...
	if (mail($arr["email"], "{$CACHEARRAY['defaultbaseurl']} восстановление пароля", wordwrap($body,70), "From: {$CACHEARRAY['siteemail']}")==false) stderr($tracker_lang['error'],"Ошибка при отправке письма");
...
EOD;
	...
	$mail_sent = mail($email, "{$CACHEARRAY['defaultbaseurl']} данные аккаунта", $body, "From: {$CACHEARRAY['siteemail']}");
