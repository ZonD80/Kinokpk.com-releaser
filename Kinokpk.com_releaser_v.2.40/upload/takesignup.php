...
$msn = unesc(htmlspecialchars($_POST["msn"]));
if (strlen($msn) > 30)
bark("Жаль, Ваш msn слишком длинный  (Макс - 30)");

$aim = unesc(htmlspecialchars($_POST["aim"]));
if (strlen($aim) > 30)
bark("Жаль, Ваш aim слишком длинный  (Макс - 30)");

$yahoo = unesc(htmlspecialchars($_POST["yahoo"]));
if (strlen($yahoo) > 30)
bark("Жаль, Ваш yahoo слишком длинный  (Макс - 30)");

$mirc = unesc(htmlspecialchars($_POST["mirc"]));
if (strlen($mirc) > 30)
...
implode(",", array_map("sqlesc", array($wantusername, $wantpasshash, $secret, $editsecret, $gender, $country, $icq, $msn, $aim, $yahoo, $skype, $mirc, $website, $email, $status))).
		", ". (!$users?UC_SYSOP.", ":""). "'". get_date_time() ."', ".sqlesc($birthday).", '{$CACHEARRAY['default_language']}', ".sqlesc($inviter).", ".sqlesc($invitedroot).")");// or sqlerr(__FILE__, __LINE__);
