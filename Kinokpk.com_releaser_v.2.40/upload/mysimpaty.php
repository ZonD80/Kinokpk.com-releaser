...
<table class="embedded" cellspacing="0" cellpadding="5" width="100%">
	<tr>
		<td class="colhead" align="center" colspan="12">Список респектов</td>
	</tr>
	<?
	unset ($admin);
	...
	while ($arr = mysql_fetch_assoc($res)) {
		...
		if (substr($arr["type"],0,7) == 'torrent') {
			$from_what = '<a href="details.php?id=' . substr($arr["type"],7,strlen($arr["type"])) . '">' . "Торрент" . '</a>';
		}
		...
		$CACHEARRAY['description'] = $arr["description"];
