...
$feeds->setDescription($CACHEARRAY['sitename'].iconv("CP1251", "UTF-8"," - ".$tracker_lang['news']." RSS 2.0"));
...
if (!is_array($ids)) $feeds->addItem(new FeedItem($CACHEARRAY['defaultbaseurl'], iconv("CP1251", "UTF-8",$tracker_lang['no_releases']), $DEFAULTBASEUR, iconv("CP1251", "UTF-8",$tracker_lang['no_releases']))); else {
	...
	foreach ($namear as $id => $tname) {
		...
		$descr .= "<tr><td valign=\"top\"><b>".$tracker_lang['genre']."</b></td><td>".$tags."</td></tr>";
		...
		$uprow = (isset($usernamear[$id]) ? ("<a href=userdetails.php?id=" . $ownerar[$id] . ">" . htmlspecialchars($usernamear[$id]) . "</a>") : "<i>".$tracker_lang['anonymous']."</i>");
		...
		if ($img2 != NULL) {
            ...
            <hr />
            <b>".$tracker_lang['posted']." </b>$usernamear[$id]<br>
            <b>".$tracker_lang['size']." </b>".mksize($sizear[$id])."<br>";

			$content .= "<b>".$tracker_lang['added']." </b>$addedar[$id]
            <hr />
                        <b>".$tracker_lang['comments']." </b>$commentsar[$id]</b><br></td><td>
            <div align=\"right\">".(!empty($catname) ? "<img src=\"{$CACHEARRAY['defaultbaseurl']}/pic/cats/$catimage\" alt=\"$catname\" title=\"$catname\" border=\"0\" />" : "")."<br/>
            [<a href=\"details.php?id=$id\" alt=\"$namear[$id]\" title=\"$namear[$id]\"><b>".$tracker_lang['preview']."</b></a>] [<a href=\"browse.php\">".$tracker_lang['full_lst_rel']."</a>]</div></td></table></td>";
			$content .= "</tr>";
			$content .= "</table>";
			//        print_r ($cat);
