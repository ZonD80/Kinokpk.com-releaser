...
if ((!is_valid_id($_GET['category'])) && (isset($_GET['category']))) stderr ($tracker_lang['error'],$tracker_lang['invalid_id']);
...
$res = sql_query("SELECT count(requests.id) FROM requests INNER JOIN categories ON requests.cat = categories.id INNER JOIN users ON requests.userid = users.id $query $filtersql") or die(mysql_error());
...
if (!$count) {
	...
	$res = sql_query("SELECT users.class, users.downloaded, users.uploaded, users.username, requests.filled, requests.filledby, requests.id, requests.userid, requests.request, requests.added, requests.hits, requests.comments, categories.name AS cat_name, categories.id AS cat_id FROM requests INNER JOIN categories ON requests.cat = categories.id INNER JOIN users ON requests.userid = users.id $query $filtersql $limit") or sqlerr(__FILE__, __LINE__);
