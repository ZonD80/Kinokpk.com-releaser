...
while ($friend = mysql_fetch_array($res))
{
	...
	$body2 = "<br /><a href=friends.php?id=$userid&action=delete&type=friend&targetid=" . $friend['id'] . " onClick=\"return confirm('Вы уверены?')\">".$tracker_lang['delete']."</a>" .
...
else
{
	...
	while ($block = mysql_fetch_array($res))
	{
		if ($i % 6 == 0)
		$blocks .= "<tr>";
		$blocks .= "<td style='border: none; padding: 4px; spacing: 0px;'>[<font class=small><a onClick=\"return confirm('Вы уверены?')\" href=friends.php?id=$userid&action=delete&type=block&targetid=" .
