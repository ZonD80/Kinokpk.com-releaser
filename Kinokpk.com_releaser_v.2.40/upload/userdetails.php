...
function maketable($res)
{
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		$ttl = ($CACHEARRAY['ttl_days']*24) - floor((gmtime() - sql_timestamp_to_unix_timestamp($arr["added"])) / 3600);
...
if ($user["status"] == "pending") stderr($tracker_lang['error'],"Этот пользователь еще не подтвердил себя по e-mail, возможно, этот аккаунт вскоре будет удален");
...
if (mysql_num_rows($r) > 0) {
	...
	while ($a = mysql_fetch_assoc($r)) {
		$ttl = ($CACHEARRAY['ttl_days']*24) - floor((gmtime() - sql_timestamp_to_unix_timestamp($a["added"])) / 3600);
		if ($ttl == 1) $ttl .= "&nbsp;час"; else $ttl .= "&nbsp;часов";
...
elseif ($user["gender"] == "3") $gender = "Не определен";
...
<table width=100% border=1 cellspacing=0 cellpadding=5>
	<tr>
		<td class=rowhead width=1%>Зарегистрирован</td>
		<td align=left width=99%><?=$joindate?></td>
	</tr>
	<tr>
		<td class=rowhead>Последний раз был на трекере</td>
	...
	<tr>
		...
		if ($user["msn"])
		print("<img src=\"pic/contact/msn.gif\" alt=\"msn\" border=\"0\" /> ".htmlspecialchars($user[msn])."<br />\n");
		if ($user["aim"])
		print("<img src=\"pic/contact/aim.gif\" alt=\"aim\" border=\"0\" /> ".htmlspecialchars($user[aim])."<br />\n");
		if ($user["yahoo"])
		print("<img src=\"pic/contact/yahoo.gif\" alt=\"yahoo\" border=\"0\" /> ".htmlspecialchars($user[yahoo])."<br />\n");
		if ($user["skype"])
		print("<img src=\"pic/contact/skype.gif\" alt=\"skype\" border=\"0\" /> ".htmlspecialchars($user[skype])."<br />\n");
		if ($user["mirc"])
		print("<img src=\"pic/contact/mirc.gif\" alt=\"mirc\" border=\"0\" /> ".htmlspecialchars($user[mirc])."\n");
		?></td>
	</tr>
	<?
	}
	if ($user["website"])
	print("<tr><td class=\"rowhead\">Сайт</td><td align=\"left\">".htmlspecialchars($user[website])."</a></td></tr>\n");
	...
	if ($invitetree)
	print("<tr valign=\"top\"><td class=\"rowhead\">Приглашенные</td><div class=\"news-wrap\"><div class=\"news-head folded clickable\">Показать</div><div class=\"news-body\">$invitetree</div></td></tr>\n");
	...
	print("<tr><td align=right><b>Связь</b></td><td align=center><form method=\"get\" action=\"message.php\">
        ...
        </form><form method=\"get\" action=\"email-gateway.php\">
	...
	if (get_user_class() >= UC_MODERATOR && $user["class"] < get_user_class())
	{
		...
		print("<tr><td class=\"rowhead\">Удалить аватар</td><td colspan=\"2\" align=\"left\"><input type=\"checkbox\" name=\"avatar\" value=\"1\"></tr>\n");	// we do not want mods to be able to change user classes or amount donated...
 	...
 	<td align=\"left\" width=\"20%\">" .
		...
		if ($warned) {
			...
			print("<td align=\"center\">На неограниченый срок</td></tr>\n");
				...
				print(" (" . get_elapsed_time(time()+strtotime($warneduntil)) . " осталось)</td></tr>\n");
			}
		} else {
