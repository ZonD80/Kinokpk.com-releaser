...
if ($CACHEARRAY['use_integration']) {
	...
	if (!$transfered){
		...
		if (!$ipbuser) {
			...
			$ipbuser = register_ipb_user($row['username'],$password, $row['email'], $row['gender'], $bdate['year'], $bdate['month'], $bdate['day'], $row['aim'], $row['icq'], $row['website'], $row['yahoo'], $row['msn'], sql_timestamp_to_unix_timestamp($row['added']), false);
		}
...
stdmsg("Вы успешно вошли!","$s<a href=\"".htmlentities($_POST['returnto'])."\">Продолжить</a>");
else
