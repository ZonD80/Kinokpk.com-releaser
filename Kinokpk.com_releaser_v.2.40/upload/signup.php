...
//stderr($tracker_lang['error'], "Наш проект осуществляется совместно с форумом $CACHEARRAY['forumname'], пройдите регистрацию на нем, а затем просто войдите под своим логином и паролем. Через 3 секунды вы будете перенаправлены.");
...
if ($_POST["agree"] != "yes") {
	...
	?>
<div align="center">
<fieldset class="fieldset"><legend>Правила релизера <?=$CACHEARRAY['sitename']?></legend>
...
<table cellpadding="4" cellspacing="0" border="0" style="width: 100%"
	...
	<tr>
		...
		<font color="red">ВНИМАНИЕ! ВЫ ТАКЖЕ ЗАРЕГЕСТРИРУЕТЕСЬ НА ФОРУМЕ <?=$CACHEARRAY['forumname']?>!!!<br />
		Если уже у вас есть аккаунт на форуме <?=$CACHEARRAY['forumname']?>,
		то войдите под своим логином и паролем.</font></td>
	</tr>
	<tr>
		<td class="tablea"
		...
		<div class="page"
			style="border-right: thin inset; padding-right: 6px; border-top: thin inset; padding-left: 6px; padding-bottom: 6px; overflow: auto; border-left: thin inset; padding-top: 1px; border-bottom: thin inset; height: 170px">
		<p><strong>Правила <?=$CACHEARRAY['sitename']?> и <u><a
			target="_blank"
			href="<?=$CACHEARRAY['forumurl']?>/index.php?act=boardrules">форума <?=$CACHEARRAY['forumname']?>
		(перейти)</a></u></strong></p>
		...
		'Регистрация'. Если вы передумали регистрироваться, нажмите <a
			href="<?=$CACHEARRAY['defaultbaseurl'];?>">здесь</a>, чтобы вернуться
		на главную страницу.</p>
		<p>Хотя модераторы и администраторы, обслуживающие <?=$CACHEARRAY['sitename'];?>,
		стараются удалять все оскорбительные и некорректные сообщения из
		трекера, все равно все сообщения просмотреть невозможно. Сообщения
		отражают точку зрения только автора, но не администрации трекера,
		соответственно только автор несет ответственность за содержание
		сообщения.</p>
	...
	<tr>
		...
		<div><label> <input class="tablea" type="checkbox" name="agree"
			value="yes"> <input type="hidden" name="do" value="register"> <strong>Я
		согласен исполнять установленные правила, посещая <?=$CACHEARRAY['sitename'];?>.</strong>
		</label></div>
		</td>
	</tr>
</table>
...
<p>
<center><input class="tableinborder" type="submit" value="Регистрация">
</center>
</form>
	<?
	stdfoot();
...
<table border="1" cellspacing=0 cellpadding="10">
	...
	<tr valign=top>
		...
		<table width=250 border=0 cellspacing=0 cellpadding=0>
			<tr>
				<td class=embedded><font class=small><?=$tracker_lang['signup_email_must_be_valid'];?></td>
			</tr>
			</font>
			</td>
			</tr>
	...
	if ($CACHEARRAY['use_captcha']) {
		...
		tr("Вы человек?",recaptcha_get_html($CACHEARRAY['re_publickey']),1,1);
	...
	tr('Это так?','<input type="checkbox" name="rulesverify" value="yes"> '.$tracker_lang['signup_i_have_read_rules'].'<br />
