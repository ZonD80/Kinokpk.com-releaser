...
'.$tracker_lang['upload_notice'].'
	...
	foreach ($images as $image) {
		...
		if (!empty($image)) $imgids[] = intval(substr($image,strlen($id),strpos($image,'.')));

	}
	// die(var_dump($images));

	for ($i = 0; $i < $CACHEARRAY['max_images']; $i++) {
		if (in_array($i,$imgids)) {$dis1 = 'disabled'; $dis2='';} else {$dis1=''; $dis2='disabled';}
		$imgcontent .= "<b>Картинка №".($i+1).": Удалить? &nbsp&nbsp<input type=\"checkbox\" name=\"delimg".($i)."\" value=\"yes\" $dis2><br/></b>&nbsp&nbsp<input type=file name=image".$i." size=80 $dis1><br/><b>Или адрес картинки (URL):</b>&nbsp&nbsp<input type=\"text\" size=\"61\" name=\"img$i\" $dis1><hr/>";
	}
	...
	while ($did = mysql_fetch_array($detid))  {
		...
		tr($did['name'].(($did['required'] == 'yes')?"<font color=\"red\">*</font>":"").($dd['spoiler']=='yes'?"<br/><small>Будет автоматически скрыто спойлером</small>":""),'<input type="text" size="'.$did['size'].'" name="val['.$did['id'].']" value="'.$did['value'].'"><hr/>'.$did['description'],1,1);

		elseif ($did['input'] == 'links')
		tr($did['name'].(($dd['required'] == 'yes')?"<font color=\"red\">*</font>":"").($dd['spoiler']=='yes'?"<br/><small>Будет автоматически скрыто спойлером</small>":""),"<textarea name=\"val[".$did['id']."]\" rows=\"3\" cols=\"60\" wrap=\"soft\">".$did['value']."</textarea><hr/>".$did['description'],1,1);
		...
		elseif ($did['input'] == 'option') {
			...
			$s .= "</select>\n";
			tr($did['name'].(($dd['required'] == 'yes')?"<font color=\"red\">*</font>":"").($dd['spoiler']=='yes'?"<br/><small>Будет автоматически скрыто спойлером</small>":""),$s,1,1);
		}
	}
	...
	$s = "<input type=\"hidden\" name=\"oldtags\" value=\"".$row['tags']."\"><table width=\"100%\"><tr><td colspan=\"2\">Выбрано тегов: <input type=\"text\" name=\"tagcount\" value=\"$tagcount\" readonly/></td></tr>";
	$tags = taggenrelist($row['category']);
	if (!$tags)
	$s .= "Нет тегов для данной категории.";
	else
	{
		$tc=0;
		foreach ($tags as $tagrow) {
			...
			$s .= ($tr?"<tr>":"")."<td><input type=\"checkbox\" name=\"tags[]\" onClick=\"this.name='tags['+tagcounter(this.checked)+']';\" value=\"$tag\"".(in_array($tag,$oldtags)?" checked":"")."> $tag</td>".(!$tr?"</tr>":"");
