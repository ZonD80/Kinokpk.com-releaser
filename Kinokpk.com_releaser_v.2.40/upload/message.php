...
<TABLE border="0" cellpadding="4" cellspacing="0" width="100%">
	...
	else
	{
		while ($row = mysql_fetch_assoc($res))
		{
			...
			echo("<TD>" .(($row['archived'] == 'yes')?"<font color=\"red\">Да</font>":"Нет") . "</TD>\n");
			if ($row['sender'] == 0) $pm_del = $CACHEARRAY['pm_delete_sys_days']; else $pm_del = $CACHEARRAY['pm_delete_user_days'];

			echo("<TD>".(($row['archived'] == 'yes')?"N/A":($pm_del-round((time()-strtotime($row['added']))/86400))." дня(ей)</TD>\n"));
			echo("<TD><INPUT type=\"checkbox\" name=\"messages[]\" title=\"".$tracker_lang['mark']."\" value=\"" . $row['id'] . "\" id=\"checkbox_tbl_" . $row['id'] . "\"></TD>\n</TR>\n");
		}
	}
	?>
	<tr class="colhead">
		<td colspan="6" align="right" class="colhead"><input type="hidden"
			name="box" value="<?=$mailbox?>"> <input type="submit" name="delete"
			title="<?=$tracker_lang['delete_marked_messages'];?>"
		...
		<input type="submit" name="markread"
			...
			type="submit" name="archive" title="Архивировать"
			value="Архивировать"
...
elseif ($action == "viewmessage") {
	...
	$subject = htmlspecialchars($message['subject']).(($message['spamid'] != 0)?" [<font color=\"red\">Помечено как спам</font>]":"");
	if (strlen($subject) <= 0)
	{
		$subject = "Без темы";
	}
...
<?php
if (($message['spamid'] !=0) && $adminview) print("<tr><td><h2>Это сообщение было помечено как спам пользователем <a href=\"userdetails.php?id=".$message['spamid']."\">".$message['username']."</a></h2></td><td><a onClick=\"return confirm('Вы уверены?')\" href=\"message.php?action=delspam&id=".sqlesc($pm_id)."\">Снять пометку</a></td></tr>");
?>
	<TR>
		<TD class="colhead" colspan="2">Тема: <?=$subject?></TD>
	</TR>
	<TR>
	...
	<TR>
		...
		if ($adminview && ($CURUSER['id'] != $message['receiver']) && ($CURUSER['id']  != $message['sender'])) {
			...
			print('<font color="red">Вы просматриваете это сообщение от прав администратора!</font> Получатель: <a href="userdetails.php?id='.$message['receiver'].'">'.$a_receiver.'</a><br/>');
		}
		print("[ <A onClick=\"return confirm('Вы уверены?')\" href=\"message.php?action=deletemessage&id=$pm_id\">Удалить</A> ]$reply [ <A href=\"message.php?action=forward&id=$pm_id\">Переслать</A> ]".(($message['spamid'] == 0)?"[ <a onClick=\"return confirm('Вы уверены, что это действительно спам?')\" href=\"message.php?action=setspam&id=$pm_id\"><font color=\"red\">Это спам!</font></a> ]":"[Помечено как спам]"));
		?></TD>
	</TR>
</TABLE>
		<?
...
required_show = new Array("тему сообщения", "сообщение");
...
<table class=main border=0 cellspacing=0 cellpadding=0>
	<tr>
		...
		<table class=message cellspacing=0 cellpadding=5>
			...
			<tr>
			<? if ($replyto) { ?>
				<td align=center><input type=checkbox name='delete' value='yes'
				<?=$CURUSER['deletepms'] == 'yes'?"checked":""?> />Удалить сообщение
				после ответа <input type=hidden name=origmsg value=<?=$replyto?>></td>
				<? } ?>
				<td align=center><input type=checkbox name='save' value='yes'
				<?=$CURUSER['savepms'] == 'yes'?"checked":""?> />Сохранить сообщение
				в отправленных</td>
			</tr>
			<tr>
				<td align="center"><input type="checkbox" name='archive' value='yes' />Архивировать
				после отправки</td>
			</tr>
			<tr>
				<td <?=$replyto?" colspan=2":""?> align=center><input type=submit
					value="Послать!" class=btn /></td>
			</tr>
		</table>
...
elseif ($action == 'takemessage') {
	...
	$last_pmres = sql_query("SELECT NOW()-added AS seconds, msg,id FROM messages WHERE sender=".$CURUSER['id']." OR poster=".$CURUSER['id']." ORDER BY added DESC LIMIT 4");
	...
	if ($last_pmrow[0]){
		...
		if ($CACHEARRAY['as_check_messages'] && ($last_pmrow[0]['msg'] == $msg) && ($last_pmrow[1]['msg'] == $msg) && ($last_pmrow[2]['msg'] == $msg) && ($last_pmrow[3]['msg'] == $msg)) {
			...
			foreach ($msgids as $msgid){
				$msgview.= "\n[url=message.php?action=viewmessage&id=$msgid]Сообщение с ID={$msgid}[/url] от пользователя ".$CURUSER['username'];
			}
			...
			if (strpos($modcomment,"Maybe spammer") === false) {
				...
				while (list($admin) = mysql_fetch_array($arow)) {
					...
					$admin, '" . get_date_time() . "', 'Пользователь [url=userdetails.php?id=".$CURUSER['id']."]".$CURUSER['username']."[/url] может быть спамером, т.к. его 5 последних посланных сообщений полностью совпадают.$msgview', 'Сообщение о спаме!', 1)") or sqlerr(__FILE__, __LINE__);
				}
				$modcomment .= "\n".get_date_time()." - Maybe spammer";
				sql_query("UPDATE users SET modcomment = ".sqlesc($modcomment)." WHERE id =".$CURUSER['id']);

			} else {
				sql_query("UPDATE users SET enabled='no', dis_reason='Spam' WHERE id=".$CURUSER['id']);
				...
				while (list($admin) = mysql_fetch_array($arow)) {
					...
					stderr("Поздравляем!","Вы успешно забанены системой за спам в Личных Сообщениях! Если вы не согласны с решением системы, <a href=\"contact.php\">подайте жалобу админам</a>.");
				}
			}
			stderr($tracker_lang['error'],"На нашем сайте стоит защита от спама, ваши 5 последних сообщений совпадают. В отсылке личного сообщения отказано. <b><u>ВНИМАНИЕ! ЕСЛИ ВЫ ЕЩЕ РАЗ ПОПЫТАЕТЕСЬ ОТПРАВИТЬ ИДЕНТИЧНОЕ СООБЩЕНИЕ, ВЫ БУДЕТЕ АВТОМАТИЧЕСКИ ЗАБЛОКИРОВАНЫ СИСТЕМОЙ!!!</u></b> <a href=\"javascript: history.go(-1)\">Назад</a>");

		}
	}
	// ANTISPAM SYSTEM END
	$pms = sql_query("SELECT COUNT(*) FROM messages WHERE (receiver = $receiver AND location=1) OR (sender = $receiver AND saved = 'yes')");
	$pms = mysql_result($pms,0);
	if ($pms>=$CACHEARRAY['pm_max']) stderr($tracker_lang['error'], "Ящик личных сообщений получателя заполнен, вы не можете отправить ему сообщение.");

	if ($save == 'yes') {
		$pms = sql_query("SELECT COUNT(*) FROM messages WHERE (receiver = ".$CURUSER['id']." AND location=1) OR (sender = ".$CURUSER['id']." AND saved = 'yes')");
		$pms = mysql_result($pms,0);
		if ($pms>=$CACHEARRAY['pm_max']) stderr("Невозможно сохранить сообщение", "Ваш ящик личных сообщений заполнен, максимальное кол-во {$CACHEARRAY['pm_max']}. Вы не можете отправить сообщение, вам необходимо очистить ящик личных сообщений");
	}
...
--
		...
		{$CACHEARRAY['sitename']}
EOD;
		$subj = "Вы получили новое ЛС от $username! - ".$CACHEARRAY['defaultbaseurl'];
		mail($usremail, $subj, $body, "From: ".$CACHEARRAY['siteemail']);
	}
	$delete = $_POST["delete"];
	if ($origmsg)
	{
...
<table class=main border=0 cellspacing=0 cellpadding=0>
	<tr>
		...
		<table border=1 cellspacing=0 cellpadding=5>
			...
			<tr>
				<td colspan="2">
				<div align="center"><?=textbbcode("message","msg","$body");?></div>
				</td>
			</tr>
			<tr>
				<td colspan="2">
		...
		<input type=hidden name=pmees value="<?=$pmees?>"> <input type=hidden
			name=n_pms value=<?=$n_pms?>></form>
		<br />
		<br />
		</div>
		</td>
...
elseif ($action == "moveordel") {
	...
	elseif ($_POST["archive"]) {
		...
		// Проверяем, были ли помечены сообщения
		if (@mysql_affected_rows() == 0) {
			stderr($tracker_lang['error'], "Сообщение не может быть архивировано! ");
		}
		else {
			header("Location: message.php?action=viewmailbox&box=" . $pm_box);
			exit();
	...
	elseif ($_POST["unarchive"]) {
		...
		// Проверяем, были ли помечены сообщения
		if (@mysql_affected_rows() == 0) {
			stderr($tracker_lang['error'], "Сообщение не может быть разархивировано! ");
		}
		else {
			header("Location: message.php?action=viewmailbox&box=" . $pm_box);
			exit();
...
elseif ($action == "forward") {
	if ($_SERVER['REQUEST_METHOD'] == 'GET') {
		...
		$body = "Оригинальное сообщение:<br/>[quote=" . $from2['username'] . "]" . format_comment($message['msg']. "[/quote]");
...
<TABLE border="0" cellpadding="4" cellspacing="0">
	...
	<TR>
		...
		$body .= "Оригинальное сообщение:[quote=" . $from . "]" . $message['msg'] . "[/quote]";
		$save = (int) $_POST['save'];
		if ($save) {
			$save = 'yes';
		}
		...
		$pms = sql_query("SELECT COUNT(*) FROM messages WHERE (receiver = ".($receiver?$receiver:$to)." AND location=1) OR (sender = ".($receiver?$receiver:$to)." AND saved = 'yes')");
		$pms = mysql_result($pms,0);
		if ($pms>=$CACHEARRAY['pm_max']) stderr($tracker_lang['error'], "Ящик личных сообщений получателя заполнен, вы не можете переслать ему сообщение.");
...
elseif ($action == 'setspam') {
	...
	while (list($admin) = mysql_fetch_array($arow)) {
		sql_query("INSERT INTO messages (poster, sender, receiver, added, msg, subject, location) VALUES(0, 0,
		$admin, '" . get_date_time() . "', '[url=message.php?action=viewmessage&id=".$pm_id."]Это сообщение[/url] было помечено пользователем [url=userdetails.php?id=".$CURUSER['id']."]".$CURUSER['username']."[/url], как содержащее спам', 'Сообщение о спаме!', 1)") or sqlerr(__FILE__, __LINE__);
	}
