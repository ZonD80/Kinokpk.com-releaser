...
<table border=1 width=550 cellspacing=0 cellpadding=5>
	...
	<tr>
		...
		<form method="get" action=browse.php><input type="text" name="search"
			size="40" value="<?= htmlspecialchars($searchstr) ?>" />&nbsp;в&nbsp<select
			name="cat">
...
$subres = sql_query("SELECT COUNT(*) FROM reqcomments WHERE request = $id");
...
if (!$count) {
	...
	$subres = sql_query("SELECT c.id, c.ip, c.text, c.user, c.added, c.editedby, c.editedat, u.avatar, u.warned, ".
		"u.username, u.title, u.class, u.donor, u.downloaded, u.uploaded, u.enabled, u.parked, u.last_access, e.username AS editedbyname FROM reqcomments c LEFT JOIN users AS u ON c.user = u.id LEFT JOIN users AS e ON c.editedby = e.id WHERE c.request = " .
