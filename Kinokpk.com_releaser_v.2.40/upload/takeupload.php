...
if ($_POST['free'] == 'yes' AND get_user_class() >= UC_MODERATOR) {
	$free = "yes";
} else {
	$free = "no";
};
...
foreach($_POST['val'] as $typeid => $content) {
	$erow = sql_query("SELECT name,isnumeric,required,input,mask FROM descr_details WHERE id =".(int) $typeid);
...
if ($CACHEARRAY['use_integration']) {
	...
	if ($image) $forumdesc = "<div align=\"center\"><a href=\"{$CACHEARRAY['defaultbaseurl']}/viewimage.php?image=".$image."\"><img alt=\"Постер для фильма (кликните для просмотра полного изображения)\" src=\"{$CACHEARRAY['defaultbaseurl']}/thumbnail.php?image=".$image."&for=forum\" border=\"0\" class=\"linked-image\" /></a></div><br />";
...
if ($_POST['nofile'] == 'yes') {
	...
	$ret = sql_query("INSERT INTO torrents (search_text, filename, owner, visible, sticky, info_hash, name, size, numfiles, type, tags, descr_type, free, images, category, save_as, added, last_action) VALUES (" . implode(",", array_map("sqlesc", array(searchfield("$shortfname $dname $torrent"), $fname, $CURUSER["id"], "yes", $sticky, $infohash, $torrent, $totallen, $filelist, $type, $tags, $descr, $free, $images, (int) $_POST["type"], $dname))) . ", '" . get_date_time() . "', '" . get_date_time() . "')");
} else {

	$torrent = htmlspecialchars(str_replace("_", " ", $torrent));

	$ret = sql_query("INSERT INTO torrents (search_text, filename, owner, visible, sticky, info_hash, name, size, numfiles, type, tags, descr_type, free, images, category, save_as, added, last_action) VALUES (" . implode(",", array_map("sqlesc", array(searchfield("$shortfname $dname $torrent"), $fname, $CURUSER["id"], "yes", $sticky, $infohash, $torrent, $totallen, count($filelist), $type, $tags, $descr, $free, $images, (int) $_POST["type"], $dname))) . ", '" . get_date_time() . "', '" . get_date_time() . "')");
}
if (!$ret) {
	if (mysql_errno() == 1062)
...
if ($CACHEARRAY['use_integration']) {
	...
	$topicfooter .= "<tr><td valign=\"top\"><b>".(($_POST['nofile'] != 'yes')?"Торрент {$CACHEARRAY['defaultbaseurl']}:":"Релиз {$CACHEARRAY['defaultbaseurl']}:")."</b></td><td><div align=\"center\">[<span style=\"color:#FF0000\"><a href=\"{$CACHEARRAY['defaultbaseurl']}/details.php?id=".$id."\">Посмотреть этот фильм на {$CACHEARRAY['defaultbaseurl']}</a></span>]</div></td></tr></table>";
...
$cat = $arr["name"];
$res = sql_query("SELECT email FROM users WHERE enabled='yes' AND notifs LIKE '%[cat$catid]%' AND notifs LIKE '%[email]%'") or sqlerr(__FILE__, __LINE__);
...
$downlink = "<a title=\"Скачать\" href=\"download.php?id=$id\"><span style=\"color: red; cursor: help;\" title=\"Скачать торрент-файл.\">СКАЧАТЬ ФАЙЛ</span></a>";
