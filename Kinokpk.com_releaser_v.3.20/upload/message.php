...
<TABLE border="0" cellpadding="4" cellspacing="0" width="100%"
	...
	if (mysql_num_rows ( $res ) == 0) {
		...
		while ( $row = mysql_fetch_assoc ( $res ) ) {
			...
			echo ("<TD><A href=\"".$REL_SEO->make_link('message','action','viewmessage','id',$row ['id'])."\">" . $subject . "</A></TD>\n");
			if ($mailbox != PM_SENTBOX) {
				echo ("<TD>$username</TD>\n");
			} else {
				echo ("<TD>$receiver</TD>\n");
...
<table class=main border=0 cellspacing=0 cellpadding=0>
	<tr>
		<td class=embedded>
		<form name="message" method="post" action="<?=$REL_SEO->make_link('message')?>"
...
<FORM action="<?=$REL_SEO->make_link('message')?>" method="post"><INPUT type="hidden"
