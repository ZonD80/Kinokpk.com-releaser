...
if ($_POST['nofile']) {
	...
	if ($_POST['nofile']) {
		...
		$ret = sql_query("INSERT INTO torrents (filename, owner, visible, sticky, info_hash, tiger_hash, name, descr, size, free, images, category, online, added, last_action, relgroup".((get_user_class() >= UC_UPLOADER)?', moderatedby':'').") VALUES (" . implode(",", array_map("sqlesc", array($fname, $CURUSER["id"], 1, $sticky, $infohash, $tiger_hash, $torrent, $descr, $totallen, $free, $images, $catsstr, $online))) . ", " . time() . ", " . time() . ", $relgroup".((get_user_class() >= UC_UPLOADER)?', '.$CURUSER['id']:'').")");
	} else {
	...
	if ($REL_CONFIG['use_integration'] && !$export_disabled) {
		// IPB TOPIC TRANSFER
		...
		if (!empty($_POST['topic'])) {
			...
			if (!$forumid) $forumid = $REL_CONFIG['not_found_export_id'];
...
if ($CRON['rating_enabled']) { $msg = sprintf($REL_LANG->say_by_key('upload_notice'),$CRON['rating_perrelease'],$id,$link); sql_query("UPDATE users SET ratingsum = ratingsum + {$CRON['rating_perrelease']} WHERE id={$CURUSER['id']}"); }
