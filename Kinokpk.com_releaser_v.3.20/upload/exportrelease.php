...
$formvalue = "<div align=\"center\"><a href=\"{$REL_CONFIG['defaultbaseurl']}/".$REL_SEO->make_link('details','id',$row['id'],'name',translit($row['name']))."\"><img width=\"200px\" src=\"{$image}\"></a></div><br /><br /><b>Полное название:</b> ".$row['name']."<br /><b>Тип:</b> ".$catstr."<br />";

$formvalue .= format_comment($row['descr']);

$formvalue .= "<b>Размер:</b> ".mksize($row['size'])."<br />";

if ($REL_CONFIG['use_integration']) $formvalue .= "<b>Релиз на форуме {$REL_CONFIG['forumname']}:</b> <a href=\"{$REL_CONFIG['forumurl']}/index.php?showtopic=".$row['topic_id']."\">{$REL_CONFIG['forumurl']}/index.php?showtopic=".$row['topic_id']."</a><br /><br />";


$formvalue .= "<div align=\"center\">Оригинал релиза на <a href=\"{$REL_CONFIG['defaultbaseurl']}/".$REL_SEO->make_link('details','id',$row['id'],'name',translit($row['name']))."\">{$REL_CONFIG['sitename']}</a></b></div>";
