...
if ($row["confirmed"]) {
	safe_redirect($REK_SEO->make_link('ok','type','confirmed'));
	exit();
}

$sec = hash_pad($row["editsecret"]);
if ($md5 != md5($sec))
stderr($REL_LANG->say_by_key('error'),'Secret does not match value');

sql_query("UPDATE users SET confirmed=1, editsecret='' WHERE id=$id AND confirmed=0");

if (!mysql_affected_rows())
stderr($REL_LANG->say_by_key('error'),'Error changing confirm status. Contact site admin please.');


logincookie($id, $row["passhash"],$row['language']);


safe_redirect($REL_SEO->make_link('ok','type','confirm'));
