...
function ipb_login($username, $forumconn=true)
{
	...
	if ($REL_CONFIG['use_integration']) {
		...
		$s = ("<iframe frameborder=\"0\" src=\"{$REL_CONFIG['forumurl']}/releaser_setcookie.php?m=$id&p=$passhash&s=$session_id&c={$REL_CONFIG['ipb_cookie_prefix']}\" width=\"0\" height=\"0\"></iframe>");
...
function ipb_logout($username)
{
	...
	if ($REL_CONFIG['use_integration']) {
		$s = ("<iframe frameborder=\"0\" src=\"{$REL_CONFIG['forumurl']}/releaser_setcookie.php?unset\" width=\"0\" height=\"0\"></iframe>");
		return $s;
	} else return '';
...

{
	...
	if ($REL_CONFIG['use_integration']) {
		...
		$second = sql_query("INSERT INTO ".$fprefix."members (id,name,email,mgroup,joined,ip_address,members_display_name,members_l_display_name,members_l_username,member_login_key,bday_day,bday_month,bday_year)
            VALUES (" .implode(",", array_map("sqlesc", array($idf,$wantusername,$email,$REL_CONFIG['defuserclass'],$time,$ip,$wantusername,$wantusername,$wantusername,$gs,$day,$month,$year))).")");
