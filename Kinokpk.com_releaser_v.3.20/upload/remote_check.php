...
$cronrow = mysql_query("SELECT * FROM cron WHERE cron_name IN ('remotecheck_disabled','remotepeers_cleantime','in_remotecheck','remote_trackers','remote_lastchecked')") or sqlerr(__FILE__,__LINE__);
...
if (!$CRON['remotecheck_disabled']) {
	...
	if ($parray) {
		...
		foreach ($parray as $id => $torrent) {
			...
			mysql_query("UPDATE LOW_PRIORITY trackers SET seeders=".(int)$peers['seeders'].", leechers=".(int)$peers['leechers'].", lastchecked=".time().", state='".mysql_real_escape_string($peers['state'])."' WHERE id=$id") or sqlerr(__FILE__,__LINE__);
		}
