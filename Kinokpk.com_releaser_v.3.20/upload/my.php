...
div(!$CURUSER['avatar'] ? "Загрузить аватар" : "Поменять аватар", "<a class=\"index\" href=\"".$REL_SEO->make_link('avatarup')."\" onclick=\"javascript:$.facebox({ajax:'".$REL_SEO->make_link('avatarup')."'}); return false;\">Кликните сюда для загрузки</a><br />\n".sprintf($REL_LANG->say_by_key('max_avatar_size'), $REL_CONFIG['avatar_max_width'], $REL_CONFIG['avatar_max_height']), 1,'my_title','up_avatar');
...
div($REL_LANG->say_by_key('my_info'), "Показывается на вашей публичной странице<br /><textarea name=info cols=57 rows=4>" . $CURUSER["info"] . "</textarea><br />", 1,'my_title','my_area');
print('<div class="clear"></div>');
...
//              $spend = "</div></div>";
div('Мои уведомления','<a href="'.$REL_SEO->make_link('mynotifs','settings','').'">Перейти на страницу конфигурации моих уведомлений</a>',1,'my_title','my_sent');
