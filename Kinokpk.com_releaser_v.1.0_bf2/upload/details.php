...
stdmsg("Ошибка", "Вы не можете подать жалобу на собственную раздачу");
  ...
  if (!$CURUSER) {
                    $pos = strpos($row['descr'],"[b]Формат файла:[/b]");

        if (($pos === false) && (strlen($row['$descr']) > 500))
            $row['descr'] = substr($row['descr'], 0, 500) . "...";  else $row['descr'] = substr($row['descr'], 0, $pos);

    tr($tracker_lang['description'], format_comment($row['descr']), 1, 1);
    tr("Хотите скачать?","<a href=\"login.php?reuturnto=".urlencode(basename($_SERVER["REQUEST_URI"]))."\">Войдите</a> или <a href=\"signup.php?returnto=".urlencode(basename($_SERVER["REQUEST_URI"]))."\">Зарегестрируйтесь</a> на ".$DEFAULTBASEURL,1,1);
    ...
    print("</table></p>\n"); stdfoot(); die; }
                ...
                if (!empty($row["descr"]))
                 ...
                 if (!empty($row['online']))
                 tr("Смотреть онлайн<br />","<form method=\"post\" action=\"online/onlinevideo.php\"><input type=\"hidden\" name=\"onlinevideo\" value=\"".$row['online']."\"><input type=\"submit\" value=\"Смотреть онлайн на Kinokpk.com\" /></form> <b>ТОЛЬКО ДЛЯ КПК НА WINDOWS MOBILE 5.0 ВЫШЕ! ФОРМАТ WMV</b>",1,1);
                if ($row["visible"] == "no")
...
if (!$count) {
  ...
  print("<div style=\"float: left; width: auto;\" align=\"left\"> :: Список комментариев Kinokpk.com</div>");
