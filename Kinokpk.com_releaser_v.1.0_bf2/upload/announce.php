...
$res = mysql_query('SELECT id, banned, filename free, seeders + leechers AS numpeers, UNIX_TIMESTAMP(added) AS ts FROM torrents WHERE info_hash = "'.$hash.'"') or err(mysql_error());
$torrent = mysql_fetch_array($res);
if (!$torrent)
	err('Torrent not registered with this tracker.');
	elseif ($res['filename'] == 'nofile') err ('It\'s non-torrent release');
...
if(($torrent["free"] =='yes') || ($a4['class'] == UC_VIP)){
$advdown = 0;
$advup = $upthis;
}else{
$advdown = $downthis;
