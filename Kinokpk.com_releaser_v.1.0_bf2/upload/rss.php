...
// get all vars
$res = sql_query("SELECT image1,id,name,descr,filename,size,category,added FROM torrents WHERE $where visible='yes' ORDER BY added DESC LIMIT 5") or sqlerr(__FILE__, __LINE__);
while ($row = mysql_fetch_row($res)){
list($img,$id,$name,$descr,$filename,$size,$cat,$added,$catname) = $row;
...
// output of all data
echo("<item><title><![CDATA[" . $name . "]]></title>\n<link>" . $link . "</link>\n<description><![CDATA[<div align=\"center\"><img alt=\"Постер для ".$name."\" src=\"". $DEFAULTBASEURL ."/thumbnail.php?image=".$img."&for=rss\"></div><br /><br />Тип (жанр): " . $category[$cat] . "<br />Размер: " . mksize($size) . "<br />Добавлен: " . $added . "<hr />" . format_comment($descr) . "\n]]></description>\n</item>\n");
}
