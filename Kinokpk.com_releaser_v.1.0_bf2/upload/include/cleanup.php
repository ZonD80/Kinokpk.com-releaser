...
function docleanup() {
	global $pm_delete_sys_days, $pm_delete_user_days, $torrent_dir, $signup_timeout, $max_dead_torrent_time, $use_ttl, $autoclean_interval, $points_per_cleanup, $ttl_days, $tracker_lang;
...
$secs_system = $pm_delete_sys_days*86400; // Количество дней
$dt_system = sqlesc(get_date_time(gmtime() - $secs_system)); // Сегодня минус количество дней
sql_query("DELETE FROM messages WHERE sender = '' AND archived = 'no' AND unread = 'no' AND added < $dt_system") or sqlerr(__FILE__, __LINE__);
//Удаляем ВСЕ прочтенные сообщения старше n дней
$secs_all = $pm_delete_user_days*86400; // Количество дней
...
//отключение предупрежденных пользователей (у тех у кого 5 звезд)
        ...
        write_log("Отключено $num пользователей (5 и более предупреждений)","","admin");
...
//GENERATE SITEMAP
require_once(FULL_PATH."/include/createsitemap.php");
?>