...
if (!isset($_GET['action'])) {
  ...
  while($stamp = mysql_fetch_array($stamparray)) {
    print("<tr><td>".$stamp['id']."</td><td><img src=\"".$DEFALUTBASEURL."/pic/stamp/".$stamp['image']."\"></td><td><input type=\"text\" name=\"sort[]\" size=\"4\" value=\"".$stamp['sort']."\"></td><td>".get_user_class_name($stamp['class'])."</td><td><a href=\"?action=edit&id=".$stamp['id']."\">E</a> | <a onClick=\"return confirm('Вы уверены?')\" href=\"?action=delete&id=".$stamp['id']."\">D</a></td></tr>");
  }
