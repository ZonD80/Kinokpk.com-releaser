...
//	$dict['value']['info']['value']['private']=bdec('i1e');  // add private tracker flag
	...
	$dict['value']['comment']=bdec(benc_str( "$DEFAULTBASEURL/details.php?id=$id")); // change torrent comment
...
/// IPB INTEGRATION ///// EDIT WIKI/POST CONTAINER ////////////
...
if ($topicid <> 0) {
$forumdesc = $image;
$cut = strpos($descr,"[center][url=$FORUMURL/index.php?showtopic=");
if ($cut === false) $forumdesc .= format_comment($descr); else
$forumdesc .= format_comment(substr($descr, 0, $cut));

$isnofilesize = mysql_query("SELECT filename,size FROM torrents WHERE id = $id");
$isnofilesize = mysql_fetch_array($isnofilesize);

$topicfooter =  "<b>Размер файла:</b> ".round($isnofilesize['size']/1024/1024)." МБ";

if ($isnofilesize['filename'] != 'nofile')
  $topicfooter .= "<br /><br />Torrent:<br />Нажмите на ссылку ниже"; else $topicfooter .= '';
  
$topicfooter .= "<br /><br /><br /><div align=\"center\">[<span style=\"color:#FF0000\"><a href=\"$DEFAULTBASEURL/details.php?id=".$id."&hit=1\">Этот фильм на Kinokpk.com</a></span>]</div>";
...
// WIKI VERSION: mysql_query("UPDATE ".$fprefix."posts SET post = '---', wiki = '".$forumdesc."' WHERE pid=".$postid);
mysql_query("UPDATE ".$fprefix."posts SET post = '".$forumdesc."' WHERE pid=".$postid);
