...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$email = sqlesc(htmlspecialchars(trim($_POST["email"])));
	$secret = mksecret();
	$passhash = sqlesc(md5($secret . $password . $secret));
	$secret = sqlesc($secret);

	sql_query("INSERT INTO users (added, last_access, secret, username, passhash, confirmed, email) VALUES(".time().", ".time().", $secret, $username, $passhash, 1, $email)");
	if (mysql_errno()==1062)
	stderr($tracker_lang['error'], $tracker_lang['unable_to_create_account']);
	$id = mysql_insert_id();
	header("Location: {$CACHEARRAY['defaultbaseurl']}/userdetails.php?id=$id");
	die;
}
