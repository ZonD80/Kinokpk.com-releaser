	...
	while ($a = mysql_fetch_assoc($res))
	{
		...
		$a["userid"] . "><b>" . $a["username"] . "</b>" .
         	...
         	"</td><td align=right$highlight>" . mksize($a["downloaded"]) .
      		...
      		"</td><td align=left>" . mkprettytime($a["added"]) . " (" .
		get_elapsed_time($a["added"],false) . " назад)</td></tr>");
	}
	print('</table>');
	end_frame();
}
...
function _torrenttable($res, $frame_caption)
{
	...
	<table width="100%">
		...
		while ($a = mysql_fetch_assoc($res))
		{
			...
			$ratio = "Inf.";
			print("<tr><td align=center>$num</td><td align=left><a href=details.php?id=" . $a["id"] . "><b>" .
...
function peerstable($res, $frame_caption)
{
	...
	print("<table width=\"100%\"><tr><td class=colhead>Rank</td><td class=colhead>Username</td><td class=colhead>Upload rate</td><td class=colhead>Download rate</td></tr>");
...
$type = (isset($_GET["type"]) && is_numeric($_GET["type"]))  ? (int) $_GET["type"] : 0;
if (!in_array($type,array(1,2,3,4)))
$type = 1;
$limit = (isset($_GET["lim"]) && is_numeric($_GET["lim"])) ? (int) $_GET["lim"] : false;
...
if ($type == 1)
{
	$mainquery = "SELECT id as userid, username, added, uploaded, downloaded, uploaded / (".time()." -added) AS upspeed, downloaded / (".time()."-added) AS downspeed FROM users WHERE enabled = 1";
...
elseif ($type == 2)
{
	...
	{
		$r = sql_query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 0 GROUP BY t.id ORDER BY seeders + leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
	...
	{
		$r = sql_query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 0 GROUP BY t.id ORDER BY times_completed DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
	...
	{
		$r = sql_query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 0 AND leechers >= 5 AND times_completed > 0 GROUP BY t.id ORDER BY data DESC, added ASC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
	...
	{
		$r = sql_query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 0 AND seeders >= 5 GROUP BY t.id ORDER BY seeders / leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
	...
	{
		$r = sql_query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 0 AND leechers >= 5 AND times_completed > 0 GROUP BY t.id ORDER BY seeders / leechers ASC, leechers DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
...
elseif ($type == 3)
{
	...
	if ($limit == 10 || $subtype == "ul")
	{
		$r = sql_query("SELECT c.name, c.flagpic, sum(u.uploaded) AS ul FROM users AS u LEFT JOIN countries AS c ON u.country = c.id WHERE u.enabled = 1 GROUP BY c.name ORDER BY ul DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
	...
	{
		$r = sql_query("SELECT c.name, c.flagpic, sum(u.uploaded)/count(u.id) AS ul_avg FROM users AS u LEFT JOIN countries AS c ON u.country = c.id WHERE u.enabled = 1 GROUP BY c.name HAVING sum(u.uploaded) > 1099511627776 AND count(u.id) >= 100 ORDER BY ul_avg DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
...
elseif ($type == 4)
{
	...
	if ($limit == 10 || $subtype == "ul")
	{
		//				$r = sql_query("SELECT users.id AS userid, peers.id AS peerid, username, peers.uploaded, peers.downloaded, peers.uploaded / (".time()." - (".time()." - last_action) - started) AS uprate, peers.downloaded / (".time()." - (".time()." - last_action) - started) AS downrate FROM peers LEFT JOIN users ON peers.userid = users.id ORDER BY uprate DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
		...
		//				$r = sql_query("SELECT users.id AS userid, peers.id AS peerid, username, peers.uploaded, peers.downloaded, (peers.uploaded - peers.uploadoffset) / (last_action - started) AS uprate, (peers.downloaded - peers.downloadoffset) / (last_action - started) AS downrate FROM peers LEFT JOIN users ON peers.userid = users.id ORDER BY uprate DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
		...
		$r = sql_query( "SELECT users.id AS userid, username, (peers.uploaded - peers.uploadoffset) / (last_action - started) AS uprate, IF(seeder = 1,(peers.downloaded - peers.downloadoffset)  / (finishedat - started),(peers.downloaded - peers.downloadoffset) / (last_action - started)) AS downrate FROM peers LEFT JOIN users ON peers.userid = users.id ORDER BY uprate DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
		peerstable($r, "Top $limit Fastest Uploaders" . ($limit == 10 && $pu ? " <font class=small> - [<a href=topten.php?type=4&amp;lim=100&amp;subtype=ul>Top 100</a>] - [<a href=topten.php?type=4&amp;lim=250&amp;subtype=ul>Top 250</a>]</font>" : ""));
	}
	...
	if ($limit == 10 || $subtype == "dl")
	{
		...
		$r = sql_query("SELECT users.id AS userid, peers.id AS peerid, username, peers.uploaded, peers.downloaded,(peers.uploaded - peers.uploadoffset) / (last_action - started) AS uprate, IF(seeder = 1,(peers.downloaded - peers.downloadoffset)  / (finishedat - started),(peers.downloaded - peers.downloadoffset) / (last_action - started)) AS downrate FROM peers LEFT JOIN users ON peers.userid = users.id ORDER BY downrate DESC LIMIT $limit") or sqlerr(__FILE__, __LINE__);
