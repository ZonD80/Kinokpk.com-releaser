...
if (!$id)
httperr();
$nof = sql_query("SELECT filename, announce_urls FROM torrents WHERE id = $id");
list($nofr,$anarray) = mysql_fetch_array($nof);

$info = bdec($info);
if (!$anarray) $anstring = $tracker_lang['distr_our_tracker']; else $anstring = 'Удаленные трекера: '.$anarray;
