...
$res = sql_query("SELECT torrents.seeders, torrents.banned, torrents.leechers, torrents.name, torrents.times_completed, torrents.id, torrents.last_reseed AS lr, categories.name AS cat_name FROM torrents LEFT JOIN categories ON torrents.category = categories.id WHERE torrents.id = $id") or sqlerr(__FILE__, __LINE__);
...
Ваша помощь необходима в раздаче <a href=details.php?id={$id}>{$row["cat_name"]} :: {$row["name"]}</a>
Если вы решили помочь, но уже удалили торрент-файл, можете скачать его <a href=download.php?id=$id>здесь</a>.
