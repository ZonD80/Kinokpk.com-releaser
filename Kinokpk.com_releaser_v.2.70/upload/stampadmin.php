...
if (!isset($_GET['action'])) {
	...
	print("<table width=\"100%\" border=\"0\"><tr><td><a href=\"stampadmin.php?action=add\">Добавить штамп</a></td></tr></table>");
	$stamparray = sql_query("SELECT * FROM stamps ORDER BY id ASC");
	print("<table width=\"100%\" border=\"1\"><tr><td class=\"colhead\">ID</td><td class=\"colhead\">Картинка</td><td class=\"colhead\">Порядок</td><td class=\"colhead\">Класс доступа</td><td class=\"colhead\">Ред/Уд</td></tr><form name=\"saveids\" action=\"stampadmin.php?action=saveids\" method=\"post\">");
	while($stamp = mysql_fetch_array($stamparray)) {
		print("<tr><td>".$stamp['id']."</td><td><img src=\"pic/stamp/".$stamp['image']."\"></td><td><input type=\"text\" name=\"sort[".$stamp['id']."]\" size=\"4\" value=\"".$stamp['sort']."\"></td><td>".get_user_class_name($stamp['class'])."</td><td><a href=\"stampadmin.php?action=edit&id=".$stamp['id']."\">E</a> | <a onClick=\"return confirm('Вы уверены?')\" href=\"stampadmin.php?action=delete&id=".$stamp['id']."\">D</a></td></tr>");
	}
...
elseif ($_GET['action'] == 'saveedit') {
	sql_query("UPDATE stamps SET image=".sqlesc($_POST['image']).",sort=".intval($_POST['sort']).",class=".intval($_POST['class'])." WHERE id=".intval($_POST['id']));
	header("Location: stampadmin.php");
	exit();
}
elseif ($_GET['action'] == 'saveadd') {

	sql_query("INSERT INTO stamps (image,sort,class) VALUES (".sqlesc($_POST['image']).",".intval($_POST['sort']).",".intval($_POST['class']).")");
	header("Location: stampadmin.php");
	exit();
}
