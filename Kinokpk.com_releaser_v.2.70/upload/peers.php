...
$cheaters = sql_query("SELECT p.torrent AS tid, t.name AS tname, p.ip, p.port, s.uploaded, s.downloaded, s.to_go, p.seeder, p.agent, p.peer_id, p.userid, u.username, u.class, u.enabled, u.warned, u.donor, (p.uploadoffset / (p.last_action - p.prev_action)) AS upspeed, (p.downloadoffset / (p.last_action - p.prev_action)) AS downspeed FROM peers AS p INNER JOIN users AS u ON u.id = p.userid INNER JOIN torrents AS t ON t.id = p.torrent INNER JOIN snatched AS s ON (s.userid = p.userid AND s.torrent = p.torrent) WHERE u.enabled = 1 ".$condition." ORDER BY upspeed DESC $limit") or sqlerr(__FILE__,__LINE__);
