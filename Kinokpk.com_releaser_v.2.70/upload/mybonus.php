...
function send(){
   ...
   $("#ajax").append('<div align="center"><img src="pic/loading.gif" border="0"/></div>');
    $.post("mybonus.php", { ajax: 1, id: bonus_type }, function(data){
   $("#ajax").empty();
   $("#ajax").append(data);
});
})(jQuery);
...
<div id="loading-layer"
	style="display: none; font-family: Verdana; font-size: 11px; width: 200px; height: 50px; background: #FFF; padding: 10px; text-align: center; border: 1px solid #000">
<div style="font-weight: bold" id="loading-layer-text">Загрузка.
Пожалуйста, подождите...</div>
<br />
<img src="pic/loading.gif" border="0" /></div>
<div id="ajax">
<table class="embedded" width="550" border="1" cellspacing="0"
	cellpadding="5">
	<?
	...
	?>
	<tr>
		<td class="colhead" colspan="3">Мой бонус (<?=$CURUSER["bonus"];?>
		бонусов в наличии / <?=$CACHEARRAY['points_per_hour'];?> бонусов в
		час)</td>
	</tr>
	<tr>
		<td class="colhead">Тип бонуса</td>
		<td class="colhead">Очки</td>
		<td class="colhead">Выбор</td>
	</tr>
	<form action="mybonus.php" name="mybonus" method="post"><?=$output;?>
	<tr>
