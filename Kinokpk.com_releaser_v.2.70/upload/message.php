...
elseif ($action == "viewmessage") {
	...
	$subject = makesafe($message['subject']).(($message['spamid'] != 0)?" [<font color=\"red\">Помечено как спам</font>]":"");
	if (strlen($subject) <= 0)
	{
		$subject = "Без темы";
	}
	...
	<tr>
		...
		<table class=message cellspacing=0 cellpadding=5>
			...
			<? if ($replyto) { ?>
				...
				<?=$CURUSER['deletepms']?"checked":""?> />Удалить сообщение после
				ответа <input type=hidden name=origmsg value=<?=$replyto?> /></td>
				<? } ?>
...
elseif ($action == "forward") {
	if ($_SERVER['REQUEST_METHOD'] == 'GET') {
		...
		$body = "Оригинальное сообщение:<hr />{$from2['username']}:<br /><blockquote><cite title=\"" . $from2['username'] . "\">" . format_comment($message['msg']. "</cite></blockquote><hr />");
...
<TABLE border="0" cellpadding="4" cellspacing="0">
	...
	<TR>
		...
		$body .= "Оригинальное сообщение:<hr />{$from}:<br /><blockquote><cite title=\"" . $from . "\">" . $message['msg'] . "</cite></blockquote><hr />";
		$save = (int) $_POST['save'];
		if ($save) {
			$save = 1;
		}
...
elseif ($action == 'setspam') {
	...
	while (list($admin) = mysql_fetch_array($arow)) {
		write_sys_msg($admin,"<a href=\"message.php?action=viewmessage&id=".$pm_id."\">Это сообщение</a> было помечено пользователем <a href=\"userdetails.php?id=".$CURUSER['id']."\">".$CURUSER['username']."</a>, как содержащее спам", 'Сообщение о спаме!');
	}
