...
"<input type=\"radio\" name=\"parked\"" . ($CURUSER["parked"] ? " checked" : "") . " value=\"1\">".$tracker_lang['yes']."
<input type=\"radio\" name=\"parked\"" . (!$CURUSER["parked"] ? " checked" : "") . " value=\"0\">".$tracker_lang['no']."
<br /><font class=\"small_text\">".$tracker_lang['my_you_can_park'].".</font>"
,1);

tr($tracker_lang['my_delete_after_reply'], "<input type=checkbox name=deletepms" . ($CURUSER["deletepms"] ? " checked" : "") . ">",1);
tr($tracker_lang['my_sentbox'], "<input type=checkbox name=savepms" . ($CURUSER["savepms"] ? " checked" : "") . ">",1);
...
if (mysql_num_rows($r) > 0)
{
	...
	while ($a = mysql_fetch_assoc($r))
	{
		...
		$categories .= "<td class=bottom style='padding-right: 5px'><input name=cat$a[id] type=\"checkbox\" " . (strpos($CURUSER['notifs'], "[cat$a[id]]") !== false ? " checked" : "") . " value='1'>&nbsp;" . htmlspecialchars($a["name"]) . "</td>\n";
		++$i;
	}
	$categories .= "</tr></table>\n";
}

tr($tracker_lang['my_email_notify'], "<input type=checkbox name=pmnotif" . (strpos($CURUSER['notifs'], "[pm]") !== false ? " checked" : "") . " value=\"1\"> Уведомить меня при получении ЛС<br />\n" .
	 "<input type=checkbox name=emailnotif" . (strpos($CURUSER['notifs'], "[email]") !== false ? " checked" : "") . " value=\"1\"> Уведомить меня при размещении торрента в одной <br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; из следующих предпочитаемых категорий.\n"
	 , 1);
	 ...
	 tr($tracker_lang['my_avatar_url'],$CURUSER['avatar']?"<img src=\"".$CURUSER['avatar']."\">":"Нет аватара",1);
	 tr("Загрузить аватар", "<a href=avatarup.php>Кликните сюда для загрузки</a><br />\n".sprintf($tracker_lang['max_avatar_size'], $CACHEARRAY['avatar_max_width'], $CACHEARRAY['avatar_max_height']), 1);
...
<input type=radio name=gender" . ($CURUSER["gender"] == "0" ? " checked" : "") . " value=0>N/A"
,1);
...
tr($tracker_lang['my_show_avatars'], "<input type=checkbox name=avatars" . ($CURUSER["avatars"] ? " checked" : "") . "> (Пользователи с маленькими каналами могут отключить эту опцию)",1);
tr("Отображать экстра-эффекты<br />Например, снег", "<input type=checkbox name=extra_ef" . ($CURUSER["extra_ef"] ? " checked" : "") . "> (Пользователи со слабыми процессорами или видеокартами могут отключить эту опцию)",1);
tr($tracker_lang['my_info'], "<textarea name=info cols=50 rows=4>" . $CURUSER["info"] . "</textarea><br />Показывается на вашей публичной странице.", 1);
...
?>
			<tr>
				<td colspan="2" align="center"><input type="submit"
					...
					value="Сбросить изменения" style='height: 25px'></td>
			</tr>
		</table>
		</form>
		</td>
