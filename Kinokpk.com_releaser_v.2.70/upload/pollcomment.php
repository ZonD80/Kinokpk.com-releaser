...
if ($action == "add")
{
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		// ANITSPAM SYSTEM END
		mysql_query("INSERT INTO pollcomments (user, poll, added, text, ip) VALUES (" .
		...
		// send_comment_notifs($pid,"<a href=polloverview.php?id=$pid#comm$newid>{$tracker_lang['poll']}</a>",'pollcomments');

		header("Location: polloverview.php?id=$pid#comm$newid");
		die;
	}
...
<table class="main" border="0" cellspacing="0" cellpadding="3">
	...
	<tr>
		...
		$res = sql_query("SELECT pollcomments.id, text, pollcomments.ip, pollcomments.ratingsum, pollcomments.added, username, title, class, users.id as user, users.avatar, users.donor, users.enabled, users.warned, users.parked FROM pollcomments LEFT JOIN users ON pollcomments.user = users.id WHERE poll = $pid ORDER BY comments.id DESC");
