...
if ($_GET['action'] == "list") {
	...
	$res2 = sql_query("SELECT userid, seeder, torrent, agent FROM peers WHERE connectable=0 ORDER BY userid DESC") or sqlerr(__FILE__, __LINE__);
...
if ($_GET['action'] == ""){
	...
	while($arr2 = mysql_fetch_assoc($getlog)){
		...
		$a2 = mysql_fetch_assoc($r2);
		$elapsed = get_elapsed_time($arr2['date']);
