...
for ($x=0; $x < $CACHEARRAY['max_images']; $x++) {
	...
	if (!empty($_POST['img'.$x])) {
		...
		if (!$check) stderr($tracker_lang['error'],'Загружаемый картинка '.$y.' - не картинка');
...
$forumdesc .= "<table width=\"100%\" border=\"1\"><tr><td valign=\"top\"><b>Тип (жанр):</b></td><td>".$forumcats."</td></tr><tr><td><b>Название:</b></td><td>" . sqlforum($torrent) ."</td></tr>";
...
if ($_POST['nofile']) {
	...
	$topicfooter .= "<tr><td valign=\"top\"><b>".((!$_POST['nofile'])?"Торрент {$CACHEARRAY['defaultbaseurl']}:":"Релиз {$CACHEARRAY['defaultbaseurl']}:")."</b></td><td><div align=\"center\">[<span style=\"color:#FF0000\"><a href=\"{$CACHEARRAY['defaultbaseurl']}/details.php?id=".$id."\">Посмотреть этот фильм на {$CACHEARRAY['defaultbaseurl']}</a></span>]</div></td></tr></table>";
...
EOD;
	...
	while ($arr = mysql_fetch_row($res))
	{
		...
		{
			sent_mail($to, $CACHEARRAY['sitename'], $CACHEARRAY['siteemail'],  "Новый релиз - $torrent", $body);
			$nthis = 0;
		}
	}
