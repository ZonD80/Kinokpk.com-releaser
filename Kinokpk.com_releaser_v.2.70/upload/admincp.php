...
<table width=100% cellspacing=10 align=center>
	...
	<tr>
		...
		<td colspan="3"><a href="cronadmin.php">Настройка cron-функций</a></td>
	</tr>
</table>
	<? end_frame();
}
...
<table width=100% cellspacing=10 align=center>
	<tr>
		...
		<td><a href="javascript://" onClick="alert('Coming soon')">Бан
		клиентов</a></td>
	...
	<tr>
		...
		<td><a href="rulesadmin.php">Редактор Правил</a></td>
	</tr>
	<tr>
	...
	<tr>
		<td><a href="pages.php">Просмотр всех страниц</a></td>
		<td colspan="3"><a href="clearcache.php">Очистка кешей</a></td>
	</tr>
</table>
<? end_frame();
}
...
<table width=100% cellspacing=3>
	...
	<tr>
		...
		<td colspan="3"><a href="findnotconnectable.php">Юзеры за NAT</a></td>
	</tr>
	<tr>
	...
	<tr>
		<td class=embedded>
		<li><a href="usersearch.php">Административный поиск</li>
		</a></td>
	</tr>
</table>
	...
	<? if ($act == "users") {
		...
		<? if ($act == "last") {
			...
			<? if ($act == "banned") {
				...
				if ($row = mysql_fetch_array($result)) {
					do {
						if (!$row["uploaded"]) { $ratio = "inf"; }
						elseif (!$row["downloaded"]) { $ratio = "inf"; }
						else {
							$ratio = number_format($row["uploaded"] / $row["downloaded"], 3);
							$ratio = "<font color=" . get_ratio_color($ratio) . ">$ratio</font>";
						}
