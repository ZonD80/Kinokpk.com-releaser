...
$content = $CACHE->get('block-indextorrents','kinopoisk');
...
if ($content===false) {
	...
	<td align="left"><div id="coloredPicturesPanel"><div id="leftNav"><div><h4 class="active">'.$tracker_lang['browse'].'</h4><ul>';
	...
	while ($row = mysql_fetch_assoc($res)) {
		...
		$content .= '<li><a href="details.php?id='.$row['id'].'">'.(($row['free'])?'<img border="0" src="pic/freedownload.gif" alt="Золотой торрент" title="Золотой торрент"/>&nbsp;':'').$row['name'].'</a><div>
  <h5>'.$row['name'].'</h5><p>'.$row['descr'].'</p>'.($filmid?'<a href="http://www.kinopoisk.ru/level/1/film/'.$filmid.'/" class="info icon">info</a> '.($flashcode?'<a href="#__ClipID='.$flashcode.'" class="trailer icon">trailer</a> ':'').' ':'').'<a href="details.php?id='.$row['id'].'" class="details icon">details</a></div><img class="changeImage" src="'.$imgcode.'" alt="'.$row['name'].'" title="'.$row['name'].'" /></li>';
		//break;
	}
	...
	$CACHE->set('block-indextorrents','kinopoisk',$content);
