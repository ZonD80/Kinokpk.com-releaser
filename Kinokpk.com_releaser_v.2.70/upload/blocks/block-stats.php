              ...
              (SELECT SUM(1) FROM users WHERE class = ".UC_VIP.") UNION ALL
               (SELECT SUM(size) FROM torrents) UNION ALL
                ...
                (SELECT SUM(uploaded) FROM users) UNION ALL
                 (SELECT SUM(remote_seeders) FROM torrents) UNION ALL
                 (SELECT SUM(remote_leechers) FROM torrents)");
...
<tr><td colspan=\"2\" class=\"rowhead\"><table width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"5\"><tr><td style=\"text-align: right; font-weight: bold; vertical-align: top;\">Мест на трекере</td><td align=\"right\">".number_format($CACHEARRAY['maxusers'])."</td></tr></table></td></tr>
