...
if (isset($_GET['delid'])) {
	...
	$CACHE->clearCache('system','categories');
...
elseif (isset($_GET['edited'])) {
	...
	$category_name = sqlesc(htmlspecialchars($_POST['category_name']));
	...
	$category_pic = sqlesc(htmlspecialchars($_POST['category_pic']));
...
while ($row = mysql_fetch_assoc($sql)) {
	...
	print("<tr><td><strong>$id</strong></td><td>$sort</td><td>$name</td><td>".($image?'<img src="pic/cats/'.$image.'"/>':$tracker_lang['no'])."</td><td>".($cats[$parent]?$cats[$parent]:$tracker_lang['no'])."</td><td>$forumid</td><td>".($disexp?$tracker_lang['no']:$tracker_lang['yes'])."</td><td><a href='category.php?editid=$id'><div align='center'><img src='pic/multipage.gif' border='0' class='special' /></a></div></td> <td><div align='center'><a onclick=\"return confirm('{$tracker_lang['confirmation_delete']}');\" href='category.php?delid=$id'><img src='pic/warned2.gif' border='0' class='special' align='center' /></a></div></td></tr>");
}
print("</table>");
stdfoot();
?>