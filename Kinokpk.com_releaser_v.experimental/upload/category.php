...
$catsrow = $REL_DB->query("SELECT id,name,seo_name FROM categories ORDER BY sort ASC");
...
if (isset($_GET['delid'])) {
    if (!is_valid_id($_GET['delid'])) {
        $REL_TPL->stdmsg($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('invalid_id'), 'error');
        $REL_TPL->stdfoot();
        die();
    }
    ...
    $REL_DB->query("DELETE FROM categories WHERE id=" . sqlesc($delid) . " LIMIT 1");
    ...
    $REL_TPL->stdmsg($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('category_success_delete'));
    $REL_TPL->stdfoot();
    die();
} ///////////////////// E  D  I  T  A  CATEGORY \\\\\\\\\\\\\\\\\\\\\\\\\\\\
elseif (isset($_GET['edited'])) {
    ...
    $REL_DB->query("UPDATE categories SET
name = $category_name,
parent_id = $category_parent, image=$category_pic, sort=$category_sort,seo_name=$category_seo WHERE id=$category_id");
    safe_redirect($REL_SEO->make_link('category'), 1);
    $REL_TPL->stdmsg($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('category_success_edit'));
    $REL_CACHE->clearGroupCache('trees');
    $REL_TPL->stdfoot();
    die();
}
...
elseif (isset($_GET['editid'])) {
    if (!is_valid_id($_GET['editid'])) {
        $REL_TPL->stdmsg($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('invalid_id'), 'error');
        $REL_TPL->stdfoot();
        die();
    }
    $cid = (int)$_GET['editid'];
    $res = $REL_DB->query("SELECT * FROM categories WHERE id=$cid");
...
if (isset($_GET['added'])) {
    ...
    $category_sort = sqlesc((int)$_POST['category_sort']);
    $category_seo = sqlesc(htmlspecialchars((string)$_POST['category_seo']));
    $REL_DB->query("INSERT INTO categories (name,image,parent_id,sort,seo_name) VALUES ($category_name,$category_pic,$category_parent,$category_sort,$category_seo)");
    safe_redirect($REL_SEO->make_link('category'), 1);
...
$sql = $REL_DB->query("SELECT * FROM categories ORDER BY parent_id ASC");
