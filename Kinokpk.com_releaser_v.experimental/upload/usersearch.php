...
?>
	...
	<table border="1" cellspacing="0" cellpadding="5">
		<tr>
			...
			<?php			$options = array($REL_LANG->_('equal to'),$REL_LANG->_('higher'),$REL_LANG->_('lover'),$REL_LANG->_('between'));
		...
		<tr>
			...
			<?php			$options = array($REL_LANG->_('at'),$REL_LANG->_('earlier than'),$REL_LANG->_('after'),$REL_LANG->_('between'));
			for ($i = 0; $i < count($options); $i++){
			...
			<td valign="middle" class="rowhead" colspan="3"><?php print $REL_LANG->_('Donated');?>:</td>
	...
	if (count($_GET) > 0 && !$_GET['h'])
	{
	  ...
	  $q[] = urlencode(trim(htmlspecialchars((string)$_GET['n'])));
...

			{
...

				{
...

					{
						...
						$REL_TPL->stdmsg($REL_LANG->say_by_key('error'), $REL_LANG->_('Secondary rating value must be greather than primary'));
		...
		if ($emaila[0] !== "")
		{
	  	...
	  	&& strpos($email,'%') === False)
	  	{
	  		if (!validemail($email))
	  		{
	  			$REL_TPL->stdmsg($REL_LANG->say_by_key('error'), $REL_LANG->_('Invalid email'));
	  			$REL_TPL->stdfoot();
	  			die();
	  		}
		...
		if ($date)
		{
			...
			else
			{
				...
				if ($datetype == "3")
				{
					...
					{
						$REL_TPL->stdmsg($REL_LANG->say_by_key('error'), $REL_LANG->_('Two date values are required for this type of search'));
						$REL_TPL->stdfoot();
						die();
					}
				}
		...
		if ($last)
		{
			...
			else
			{
				...
				if ($lasttype == "3")
				{
					...
					{
						$REL_TPL->stdmsg($REL_LANG->say_by_key('error'), $REL_LANG->_('Invalid secondary date'));
						$REL_TPL->stdfoot();
						die();
					}
				}
  	...
  	u.class, u.donor, u.modcomment, u.enabled, u.warned";
		...
		$REL_TPL->stdmsg($REL_LANG->_('Warning'),$REL_LANG->_('No users found'));
		else
		{
			print "<table border=1 cellspacing=0 cellpadding=5>\n";
			print "<tr><td class=colhead align=left>{$REL_LANG->_('Username')}</td>
        ...
        "<td class=colhead align=left>{$REL_LANG->_('Last activity')}:</td>".
	...
	<table border="1" cellpadding="5" cellspacing="0">
		<tr>
			<td>
			<div align="center"><?php print $REL_LANG->_('Send private messages for these users');?><br />
