...
<table width="100%" style="border-style: none" align="center"
	...
	<tr>
		...
		// NOTIFICATION PANEL (SOME CSS WANTED)
		$array = generate_notify_array();
	?>
<li class="mainitems">
<div class="headerlinks"><h2><a href="<?php print $REL_SEO->make_link('mynotifs');?>"><?php print $REL_LANG->_("Notifications"); if ($array['total']) print " ({$array['total']})";?></a></h2></div>
<?php 
	foreach ($array['notifs'] as $notify => $ncount) {
	$notifyout.= "<li><a href=\"{$REL_SEO->make_link('mynotifs','type',$notify)}\">".$REL_LANG->_(ucfirst($notify)).": $ncount</a></li>";
	}
	if ($notifyout) print '<ul id="submenus">'.$notifyout.'</ul>';
	/// END	
		?>
		print("</td></tr></table>\n");
