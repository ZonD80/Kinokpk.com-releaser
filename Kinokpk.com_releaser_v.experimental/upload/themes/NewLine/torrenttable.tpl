...
<a class="altlink_white">Тип</a>
			...
			{/if}
			<td class="colhead center"><a class="altlink_white" href="{$REL_SEO->make_link($orderlink.comments)}"><img title="Сортировка по количеству комментов" src="pic/browse/comments.gif" border="0"></a></td>
			...
			{/if}
			<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.size)}"><img title="Сортировка по размеру" src="pic/browse/size_file.gif" border="0"></a></td>
			<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.seeders)}"><img title="Сортировка по количесту пиров" src="pic/browse/up.gif" border="0"></a><a href="{$REL_SEO->make_link($orderlink.leechers)}"><img title="Сортировка по количесту сидов" src="pic/browse/down.gif" border="0"></a></td>
			{if $IS_MODERATOR&&$TABLE_VARIANT=='index'}
			<td class="colhead center">{$REL_LANG->say_by_key('uploadeder')}</td>
			<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.moderated)}">{$REL_LANG->_("Changed")}</a></td>
			<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.moderatedby)}">{$REL_LANG->_("Checked")}</a></td>
			<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.banned)}">{$REL_LANG->_("Banned")}</a></td>
			<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.visible)}">{$REL_LANG->_("Hide")}</a></td>
			{/if} {if $TABLE_VARIANT=='bookmarks'}
			<td class="colhead center">{$REL_LANG->say_by_key('delete')}</td>
			{/if}
<td class="colhead center"><a href="{$REL_SEO->make_link($orderlink.added)}">{$REL_LANG->say_by_key('added')}</a></td>
		</tr>
