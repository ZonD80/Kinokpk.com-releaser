...
<form action="{$REL_SEO->make_link('blocksadmin','a','reorder')}"
	method="post">
<table width="100%">
	<tr>
	...
	<tr id="block-{$block.bid}">
		<td>{$block.bid}</td>
		<td>{$block.title}</td>
		<td><input type="text" name="order[{$block.bid}]" size="3"
			value="{$block.weight}" /></td>
		<td><a href="javascript:reactivate({$block.bid});">
		<div id="act-{$block.bid}">{if
		$block.active==1}{$REL_LANG->_('Yes')}{else}{$REL_LANG->_('No')}{/if}</div>
		</a></td>
		<td>{if
	...
	<tr>
		<td colspan="10" align="right"><input type="submit"
			value="{$REL_LANG->_('Reorder blocks')}" /></td>
	</tr>
</table>
</form>
