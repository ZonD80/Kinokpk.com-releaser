...
<table id="forumtable">
	<tbody id="highlighted">
		...
		<tr>
			<td colspan="5" class="label">{$REL_LANG->_('Category')}: <a
				href="{$REL_SEO->make_link('forum','cat',$catid,'name',$catname)}">{$catname}</a></td>
		</tr>
		...
		<tr>
			...
			<td><a
				href="{$REL_SEO->make_link('forum','a','viewtopic','id',$FORUMSDATA.$fid.id,'subject',$FORUMSDATA.$fid.subject|translit,'p',"{$FORUMSDATA.$fid.lastposted_id}#comm{$FORUMSDATA.$fid.lastposted_id}")}">{$FORUMSDATA.$fid.subject}</a></td>
			<td>{$FORUMSDATA.$fid.added|mkprettytime} {$REL_LANG->_('From')}
			{$FORUMSDATA.$fid.user}<br />
			<a
				href="{$REL_SEO->make_link('forum','a','viewtopic','id',$FORUMSDATA.$fid.id,'subject',$FORUMSDATA.$fid.subject|translit,'p',"{$FORUMSDATA.$fid.lastposted_id}#comm{$FORUMSDATA.$fid.lastposted_id}")}">{$REL_LANG->_('Go')}</a></td>
		</tr>
		{else}
		<td>0</td>
		<td>0</td>
