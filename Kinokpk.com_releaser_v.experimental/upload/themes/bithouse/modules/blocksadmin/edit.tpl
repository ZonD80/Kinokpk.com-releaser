...
<table>
	...
	<tr>
		...
		({$REL_LANG->_("Will open list of pages, if none, block will appear on any page")})</div>
	...
	<tr>
		...
		{$REL_LANG->_('Check nothing to allow all')}:</td>
		<td>{$user_classes}</td>
	</tr>
	<tr>
		<td class="rowhead">{$REL_LANG->_('Content')}:<br />
		<small><a href="javascript:wysiwygjs();">{$REL_LANG->_('Enable WYSIWYG editor')}</a></small></td>
		<td>{$block.content}</td>
	</tr>
	<tr>
