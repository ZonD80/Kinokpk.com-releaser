...
<table id="torrenttable" class="center" cellspacing="0" width="100%">
	<tbody id="highlighted">
		...
		<tr class="top">
			...
			<td class="colhead center">{$REL_LANG->_('Tags')}</td>
			...
			<td class="colhead center">{$REL_LANG->_("Does not visible on main page")}</td>
 ...
 {foreach item=row from=$res} {if $row.label}
		...
		<tr {if $row.sticky} class="highlight"{/if}>
			...
			&nbsp;&nbsp;</td>
			<td class="left">{$row.tags}</td>
			<td class="left">{if $row.sticky}{$REL_LANG->_("Sticky")}: {/if}
			<div class="name_browse"><a class="download"
				href="{$REL_SEO->make_link("
				download","id",$row.id,"name",translit($row.name))}" onclick="javascript:$.facebox({ ajax:'{$REL_SEO->make_link("download","id",$row.id,"name",translit($row.name))}'}); returnfalse;"><img
				src="pic/download.gif" border="0"
				title="{$REL_LANG->say_by_key('download')}" /></a> <a
				href="{$REL_SEO->make_link("details","id",$row.id,"name",translit($row.name))}"><b>{$row.name}{$row.relgroup}</b></a>
			{if $row.new}&nbsp;<img title="{$REL_LANG->say_by_key('new')}"
				src="pic/new.png" />{/if}</div>
			{if $CURUSER.id == $row.owner || $IS_MODERATOR}<small><a id='descr'
				href="{$REL_SEO->make_link("edit","id",$row.id)}"><img border="0"
				src="pic/pen.gif" title="{$REL_LANG->say_by_key('edit')}" /></a></small>{/if}
			{if $TABLE_VARIANT<>'bookmarks'&&$CURUSER} <small><a
				href="{$REL_SEO->make_link("bookmark","torrent",$row.id,"name",translit($row.name))}"><img
				border="0" src="pic/bookmark.gif"
				title="{$REL_LANG->say_by_key('bookmark_this')}" /></a></small>{/if}
			{if $row.images}
			<div class="cat_pic"><small><a
				href="javascript:$.facebox({ image:'{$row.images}'});"><img
				border="0" src="pic/poster.gif"
				title="{$REL_LANG->say_by_key('poster')}" /></a></small></div>
			{/if} {if $row.cat_names}
			...
			<td class="center">{if !$row.moderated}<font color="green"><b>{$REL_LANG->say_by_key('no')}</b></font>{else}<font
				color="red"><b>{$REL_LANG->say_by_key('yes')}</b></font>{/if}</td>
			<td class="center">{if !$row.moderatedby}<font color="red"><b>{$REL_LANG->say_by_key('no')}</b></font>{else}<a
				href="{$REL_SEO->make_link("userdetails","id",$row.moderatedby,"username",translit($row.modname))}">{get_user_class_color($row.modclass,$row.modname)}</a>{/if}</td>
			<td class="center">{if !$row.banned}<font color="green"><b>{$REL_LANG->say_by_key('no')}</b></font>{else}<font
				color="red"><b>{$REL_LANG->say_by_key('yes')}</b></font>{/if}</td>
			<td class="center">{if $row['visible']}<font color="green"><b>{$REL_LANG->say_by_key('no')}</b></font>{else}<font
				color="red"><b>{$REL_LANG->say_by_key('yes')}</b></font>{/if}</td>
			{/if} {if $TABLE_VARIANT=='bookmarks'}
			<td class="center"><input type="checkbox" name="delbookmark[]"
				value="{$row.id}" /></td>
			{/if}
