...
<form action="{$REL_SEO->make_link('seoadmin','a','reorder')}"
	method="post">
<table width="100%">
	<tr>
	...
	<tr id="rule-{$rule.id}">
		<td>{$rule.id}</td>
		<td><input type="text" name="order[{$rule.id}]" size="3"
			value="{$rule.sort}" /></td>
		<td><a href="javascript:reactivate({$rule.id});">
		<div id="act-{$rule.id}">{if
		$rule.enabled==1}{$REL_LANG->_('Yes')}{else}{$REL_LANG->_('No')}{/if}</div>
		</a></td>
	...
	<tr>
		<td colspan="10" align="right"><input type="submit"
			value="{$REL_LANG->_('Reorder rules')}" /></td>
	</tr>
</table>
</form>
