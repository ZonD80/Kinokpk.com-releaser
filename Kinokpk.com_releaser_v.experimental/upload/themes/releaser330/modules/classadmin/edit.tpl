<form method="post" action="{$REL_SEO->make_link('classadmin','a',$a)}">
{if $a=='edit'}<input type="hidden" name="id" value="{$c.id}"/>{/if}
<table border="1">
...
<tr><td class="rowhead">{$REL_LANG->_('Priority')}</td><td><input type="text" name="c[priority]" value="{$c.prior}"></td></tr>
...
<tr><td class="rowhead">{$REL_LANG->_('Roles')}</td><td>{make_role_checkbox($c.remark,'c[role]')}<br/><small>{rolesdesc()}</small></td></tr>
