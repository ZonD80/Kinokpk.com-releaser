...
{foreach from=$TOPICS item=t}
		<tr>
			<td><a href="{$REL_SEO->make_link('forum','a','viewtopic','id',$t.id,'subject',$t.subject,'p',"{$t.lastposted_id}")}">{$t.subject}</a></td>
			<td  class="center">{$t.posts}</td>
			<td>{$t.started}</td>
			<td>{$t.author}</td>
			<td>{$t.lastposted_time}<br/>{$t.lastposted_user} 
<a href="{$REL_SEO->make_link('forum','a','viewtopic','id',$t.id,'subject',$t.subject,'p',"{$t.lastposted_id}#comm{$t.lastposted_id}")}">{$REL_LANG->_('Go')}</a></td>
			<td  class="center">{if $t.closed}{$REL_LANG->_('Yes')}{else}{$REL_LANG->_('No')}{/if}</td>
		</tr>
{/foreach}
</table>
{$pagerbottom}
