...
if (!$count) {
    ...
    $limit = ajaxpager(12, $count, array('blocks/block-indextorrents'), 'releases-table');
    if ($resarray === false) {
        $resarray = array();
    ...
    foreach ($resarray as $row) {
        ...
        $content .= "<div align=\"center\"><a href=\"" . $REL_SEO->make_link('details', 'id', $row['id'], 'name', translit($row['name'])) . "\"><img border=\"0\" src=\"$image\" width=\"155\" height=\"190\" title=\"{$row['name']}\" alt=\"{$row['name']}\"/></a>";
