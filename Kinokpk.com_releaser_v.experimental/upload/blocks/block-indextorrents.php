<?php
global $REL_LANG, $REL_CACHE, $REL_CONFIG, $REL_CONFIG, $CURUSER, $REL_SEO, $REL_DB;
...
if (!$count) {
    $content = "<div align=\"center\">{$REL_LANG->_('No releases yet')}</div>";
} else {
    ...
    if ($resarray === false) {
        $resarray = array();
        $res = $REL_DB->query("SELECT id,name,descr,images,seeders,leechers,free,category FROM torrents WHERE visible=1 AND banned=0 AND moderatedby<>0 ORDER BY added DESC $limit");
    ...
    foreach ($resarray as $row) {
        ...
        if ($row['category'] && $REL_CONFIG['pron_cats'] && !$CURUSER['pron']) {
            ...
            foreach ($categories as $category)
                if (in_array($category, $REL_CONFIG['pron_cats'])) {
                    $pron = true;
                    break;
                }
        }
        ...
        if ($pron) {
            $image = 'pic/nopron.gif';
            $row['name'] = $REL_LANG->say_by_key('xxx_release');
        } else
            if ($row['images']) $image = array_shift(explode(",", $row['images'])); else $image = 'pic/noimage.gif';


        //  $content .= "<span class=\"fl mr10\"><a href=\"{$REL_SEO->make_link('details','id',$row['id'],'name',translit($row['name']))}\"><img border=\"0\" src=\"$image\" width=\"170\" class=\"corners\" height=\"200\" title=\"{$row['name']}\" alt=\"{$row['name']}\"/></a></span><h5><b><a href=\"{$REL_SEO->make_link('details','id',$row['id'],'name',translit($row['name']))}\">{$row['name']}</a></b></h5><br/><h5> <div align=\"left\">".format_comment($row['descr'])."</div></h5><div class=\"clear5\"></div><div class=\"lineGrayDot\"></div><div class=\"clear5\"></div>";

        $content .= "<td  valign=\"top\"><div class=\"blProgrammsMain mr10\"><div class=\"clear10\"></div><a href=\"{$REL_SEO->make_link('details','id',$row['id'],'name',translit($row['name']))}\"><img border=\"0\" src=\"$image\" width=\"170\" class=\"corners\" height=\"200\" title=\"{$row['name']}\" alt=\"{$row['name']}\"/></a><div class=\"clear10\"></div><h5>" . (($row['free']) ? '<img border="0" src="pic/freedownload.gif" alt="' . $REL_LANG->_('Golden release') . '"/>&nbsp;' : '') . "<a href=\"{$REL_SEO->make_link('details','id',$row['id'],'name',translit($row['name']))}\"><b>{$row['name']}</b></br><center><img src=\"./themes/{$REL_CONFIG['ss_uri']}/images/arrowup.gif\" alt=\"{$REL_LANG->_('Seeding')}\" title=\"{$REL_LANG->_('Seeding')}\" border=\"0\" align=\"bottom\"/>&nbsp;{$row['seeders']}&nbsp;&nbsp;<img src=\"./themes/{$REL_CONFIG['ss_uri']}/images/arrowdown.gif\" alt=\"{$REL_LANG->_('Leeching')}\" title=\"{$REL_LANG->_('Leeching')}\" border=\"0\" align=\"bottom\"/>&nbsp;{$row['leechers']}</center></a></h5><div class=\"clear5\"></div><div class=\"clear10\"></div></div>";
