...
if ($CURUSER) {
    ...
    if (!$id) {
        $content .= "<h1>{$REL_LANG->_('There are no polls')}!</h1>" . ((get_privilege('polls_operation', false)) ? "[<a href=\"" . $REL_SEO->make_link('pollsadmin', 'action', 'add') . "\">{$REL_LANG->_('Create')}</a>]" : "");
    } else {
  ...
  <input type=\"hidden\" name=\"type\" value=\"$ptype\" />" . $sidvals[$sidkey];
            ...
            $content .= "</li><li class=\"polls_l\"><img src=\"./themes/{$REL_CONFIG['ss_uri']}/images/bar_left.gif\" alt=\"left\" /><img src=\"./themes/{$REL_CONFIG['ss_uri']}/images/bar.gif\" alt=\"center\" height=\"12\" width=\"" . round($percentpervote * $votecount[$vsid]) . "%\" /><img src=\"./themes/{$REL_CONFIG['ss_uri']}/images/bar_right.gif\" alt=\"right\" />&nbsp;&nbsp;$percent%, {$REL_LANG->_('Amount of votes')}: " . $votecount[$vsid] . "</li></ul>";
        }
