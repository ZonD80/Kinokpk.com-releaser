...
if (!is_valid_id($_GET["from"])) $REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('invalid_id'));
$from = (int)$_GET["from"];

if(!$from || $_GET["to"] <> "next" && $_GET["to"] <> "pre")
$REL_TPL->stderr($REL_LANG->say_by_key('error'),$REL_LANG->say_by_key('how_did_get_here'));
...
$to = $REL_DB->query("SELECT id FROM torrents WHERE id " .$pass. " LIMIT 1");
$to = @mysql_fetch_assoc($to);

$to = $to["id"];

if(!$to)
$REL_TPL->stderr($REL_LANG->say_by_key('error'), $err);
