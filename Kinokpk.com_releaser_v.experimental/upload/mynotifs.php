...
if (!$type) {
    ...
    if ($type == 'unread') {
        safe_redirect($REL_SEO->make_link('message'));
    ...
    $allowed_links = array( /*'unread' => $REL_SEO->make_link('message','action','viewmessage','id',''), */
        'torrents' => $REL_SEO->make_link('details', 'id', ''), 'relcomments' => $REL_SEO->make_link('details', 'id', ''), 'pollcomments' => $REL_SEO->make_link('polloverview', 'id', ''), 'newscomments' => $REL_SEO->make_link('newsoverview', 'id', ''), 'usercomments' => $REL_SEO->make_link('userdetails', 'id', ''), 'reqcomments' => $REL_SEO->make_link('requests', 'id', ''), 'rgcomments' => $REL_SEO->make_link('relgroups', 'id', ''), 'users' => $REL_SEO->make_link('userdetails', 'id', ''), 'reports' => $REL_SEO->make_link('reports', 'id', ''), 'unchecked' => $REL_SEO->make_link('details', 'id', ''), 'friends' => $REL_SEO->make_link('userdetails', 'id', ''));
    ...
    if ($type == 'reports') {
        $addition = 'reports.id, NULL, reports.type, reports.userid, users.username, users.class, reports.added';
        $from = 'FROM reports LEFT JOIN users ON reports.userid=users.id ORDER BY reports.added DESC';
    } elseif ($type == 'torrents') {
        $addition = 'torrents.id, torrents.id, torrents.name, torrents.owner, users.username, users.class, torrents.added';
        $from = 'FROM torrents LEFT JOIN users ON torrents.owner=users.id WHERE torrents.added>' . $CURUSER['last_login'] . ' AND torrents.moderatedby<>0 ORDER BY torrents.added DESC';
    }
    elseif ($type == 'unchecked') {
        $addition = 'torrents.id, torrents.id, torrents.name, torrents.owner, users.username, users.class, torrents.added';
        $from = 'FROM torrents LEFT JOIN users ON torrents.owner=users.id WHERE torrents.moderatedby=0 ORDER BY torrents.added DESC';
    }
    // unchecked done
    /* case 'rgcomments':
      ...
      */
    elseif ($type == 'users') {
        $addition = "users.id, NULL, NULL, users.id, users.username, users.class, users.added";
        $from = "FROM users WHERE users.added>{$CURUSER['last_login']} ORDER BY users.added DESC";
    }
    elseif ($type == 'friends') {
        $addition = "friends.userid, friends.id, friends.id, friends.userid, users.username, users.class, NULL";
        $from = "FROM friends LEFT JOIN users ON friends.userid=users.id WHERE friends.confirmed=0 AND friends.friendid={$CURUSER['id']}";
    }
    elseif (in_array($type, array('relcomments', 'pollcomments', 'newscomments', 'usercomments', 'reqcomments', 'rgcomments'))) {
        ...
        $addition = "CONCAT_WS('#comm',comments.toid,comments.id) AS cid, NULL, {$name_fields[$type]}, comments.user, users.username, users.class, comments.added";
        $from = "FROM comments LEFT JOIN users ON comments.user = users.id{$leftjoin_fields[$type]} WHERE comments.added>{$CURUSER['last_login']} AND comments.type='$typeq'" . ($type == 'forumcomments' ? " AND FIND_IN_SET(" . get_user_class() . ",forum_categories.class)" : '') . " ORDER BY comments.added DESC";
    }
    $limited = 50;
