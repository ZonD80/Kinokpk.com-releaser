...
<div id="relgroups_header" class="relgroups_header">
	...
	<div align="right" style="margin-top: -22px;"><a href="<?=$REL_SEO->make_link('relgroups');?>"><img	src="/themes/kinokpk/images/strelka.gif" border="0"	title="Вернуться к просмотру групп"	style="margin-top: 5px; margin-right: 5px;" /></a></div>

</div>
<div id="relgroups_table_right">
	<div id="relgroups_image" class="relgroups_image_right">
		<div class="relgroups_avatar_right"><img src="<?=$relgroup['image']?>" title="<?=makesafe($relgroup['name'])?>" /><? $REL_LANG->say_by_key('no_image')?>
...
<div id="relgroups_table_left">
	<div id="boxes_left" class="box_left">
		<div id="box_left" style="margin-top: 9px;">
			...
			<?print('<div align="center">'.(((get_user_class() >= UC_ADMINISTRATOR) || $I_OWNER) ? "<a class=\"box_editor_left\" title=".$REL_LANG->say_by_key('edit')." href=\"".$REL_SEO->make_link('rgnews','action','edit','id',$relgroup['id'],'newsid',$rgnewsid,'returnto',$REL_SEO->make_link('rgnews','id',$rgnewsid))."\"></a>" : "").'</div>');?></h3>
...
if (!$count) {
	...
	$subres = sql_query("SELECT nc.type, nc.id, nc.ip, nc.text, nc.ratingsum, nc.user, nc.added, nc.editedby, nc.editedat, u.avatar, u.warned, ".
                  "u.username, u.title, u.class, u.donor, u.enabled, u.ratingsum AS urating, u.gender, sessions.time AS last_access, e.username AS editedbyname FROM comments AS nc LEFT JOIN users AS u ON nc.user = u.id LEFT JOIN sessions ON nc.user=sessions.uid LEFT JOIN users AS e ON nc.editedby = e.id WHERE nc.toid = " .
                  "".$rgnewsid." AND nc.type='rgnews' ORDER BY nc.id $limit") or sqlerr(__FILE__, __LINE__);
