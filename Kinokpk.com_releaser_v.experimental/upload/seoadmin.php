...
if ($action=='react') {
	$curact = @mysql_result($REL_DB->query("SELECT enabled FROM seorules WHERE id=$id"),0);
	$REL_DB->query("UPDATE seorules SET enabled=".($curact?0:1)." WHERE id=$id");
	$REL_CACHE->clearCache('system','seorules');
	headers(REL_AJAX);
	die($curact?$REL_LANG->_("No"):$REL_LANG->_("Yes"));
} elseif ($action=='delete') {
	$REL_DB->query("DELETE FROM seorules WHERE id=$id");
	$REL_CACHE->clearCache('system','seorules');
	$REL_TPL->stderr($REL_LANG->_("Successful"),$REL_LANG->_("Rule deleted"),'success');
	...
	foreach ($arr as $bid=>$order) {
		$REL_DB->query("UPDATE seorules SET sort=$order WHERE id=".(int)$bid);
	}
	$REL_CACHE->clearCache('system','seorules');
	safe_redirect($REL_SEO->make_link("seoadmin"),1);
	$REL_TPL->stderr($REL_LANG->_("Successful"),$REL_LANG->_("Parametres order saved"),'success');
}
...
if (!$action) {
	$array = array();
	$res = $REL_DB->query("SELECT * FROM seorules ORDER BY script,parameter DESC, sort ASC");
...
elseif ($action=='edit'||$action=='add') {
	if ($action=='edit') {
		$rule = $REL_DB->query("SELECT * FROM seorules WHERE id=$id");
...
elseif ($action=='saveedit'||$action=='saveadd') {
	...
	if ($action=='saveadd') {
		$REL_DB->query("INSERT INTO seorules (".implode(',',array_keys($arr)).") VALUES (".implode(',',array_values($arr)).")");
	} else {
		foreach ($arr as $k=>$a) $to_query[] = "$k=$a";
		$REL_DB->query("UPDATE seorules SET ".implode(',',$to_query)." WHERE id=$id");
	}
	$REL_CACHE->clearCache('system','seorules');

	if (!mysql_errno()) $REL_TPL->stdmsg($REL_LANG->_("Successful"),$REL_LANG->_("Rule saved"),'success');
	else $REL_TPL->stdmsg($REL_LANG->_("Error"),$REL_LANG->_("Rule does not saved due MySQL error:").' '.mysql_error());
	$REL_TPL->end_frame();
	$REL_TPL->stdfoot();
	die();
}
