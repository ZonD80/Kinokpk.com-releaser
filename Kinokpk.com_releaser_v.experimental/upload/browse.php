...
if (!$resarray) $REL_TPL->stderr($REL_LANG->say_by_key('error'), "{$REL_LANG->_('Nothing was found')}. <a href=\"javascript: history.go(-1)\">{$REL_LANG->_('Go back')}</a>");

if (!pagercheck()) {
    $REL_TPL->stdhead($REL_LANG->say_by_key('browse'));

    $REL_TPL->begin_frame($REL_LANG->_('Releases list') . ' ' . ($modview ? '[<a href="' . $REL_SEO->make_link('browse', 'unchecked', '') . '">' . $REL_LANG->_('Show only unchecked releases') . '</a>]' : ''));
