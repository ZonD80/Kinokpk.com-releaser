...
$res = $REL_DB->query("SELECT passhash, editsecret, confirmed, language FROM users WHERE id = $id");
...
if ($row["confirmed"]) {
	$REL_TPL->stderr($REL_LANG->_("Error"),$REL_LANG->_("Your account was already confirmed."));
}

$sec = hash_pad($row["editsecret"]);
if ($md5 != md5($sec))
$REL_TPL->stderr($REL_LANG->say_by_key('error'),$REL_LANG->_('Invalid confirmation code'));

$REL_DB->query("UPDATE users SET confirmed=1, editsecret='' WHERE id=$id AND confirmed=0");

if (!mysql_affected_rows())
$REL_TPL->stderr($REL_LANG->say_by_key('error'),$REL_LANG->_('Error changing confirm status. Contact site admin please.'));
