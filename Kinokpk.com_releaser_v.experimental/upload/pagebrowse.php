...
list($pagertop, $pagerbottom, $limit) = pager($REL_CONFIG['torrentsperpage'], $count, $REL_SEO->make_link('pagebrowse').$addparam);
...
while ($resvalue = mysql_fetch_array($res)) {
	...
	if ($childs) {
		...
		if (($catq['id'] != $child['id']) && in_array($child['id'],$cats)) $chsel[]="<a href=\"".$REL_SEO->make_link('pagebrowse','cat',$child['id'],'name',translit($child['name']))."\">".makesafe($child['name'])."</a>";
...
print("<form action=\"".$REL_SEO->make_link('pagebrowse')."\" method=\"get\" style=\"width: 286px;\">".gen_select_area('cat',$tree,$cat,true)."<input type=\"submit\" class=\"button\" value=\"{$REL_LANG->say_by_key('search')}\"/></form>\n");
print('	<form method="get" action="'.$REL_SEO->make_link('pagebrowse').'" style="width: 286px;">
