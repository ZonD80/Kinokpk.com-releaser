...
elseif (isset($_POST['save'])) {
	...
	foreach ($reqparametres as $param) {
		if (!isset($_POST[$param]) && (($param != 'rating_enabled') || ($param != 'delete_votes') || ($param != 'remote_trackers')))  { $REL_TPL->stdmsg($REL_LANG->say_by_key('error'),$REL_LANG->_('Field %s does not filled',$param),'error'); $REL_TPL->stdfoot(); die; }
	...
	if ($_POST['remotecheck_disabled'] == 0) {
		foreach ($multi_param as $param) {
			if (!$_POST[$param] || !isset($_POST[$param])) { $REL_TPL->stdmsg($REL_LANG->say_by_key('error'),$REL_LANG->_('Some fields of multitracker settings does not filled'),'error'); $REL_TPL->stdfoot(); die; }
			$updateset[] = "UPDATE cron SET cron_value=".$REL_DB->sqlesc($_POST[$param])." WHERE cron_name='$param'";
		}
	}
	...
	if ($_POST['rating_enabled']) {
		foreach ($rating_param as $param) {
			if (!isset($_POST[$param])) { $REL_TPL->stdmsg($REL_LANG->say_by_key('error'),$REL_LANG->_('Some fields of rating system settings does not filled'),'error'); $REL_TPL->stdfoot(); die; }
			$updateset[] = "UPDATE cron SET cron_value=".$REL_DB->sqlesc($_POST[$param])." WHERE cron_name='$param'";
		}
	}
