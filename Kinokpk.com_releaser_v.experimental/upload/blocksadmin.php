...
function BlocksNavi() {
	global $REL_SEO, $REL_LANG;
	stdhead($REL_LANG->_("Blocks admin panel"));
	echo "<h2>{$REL_LANG->_("Blocks admin panel")}</h2><br />"
	."[ <a href=\"".$REL_SEO->make_link('blocksadmin')."\">{$REL_LANG->_("Main")}</a>"
	." | <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksNew')."\">{$REL_LANG->_("Add new block")}</a>"
	." ]";
}
...
function BlocksAdmin() {
	...
	while (list($bid, $bkey, $title, $bposition, $weight, $active, $blockfile, $view, $expire, $action, $con1, $bposition1, $weight1, $con2, $bposition2, $weight2) = mysql_fetch_row($result)) {
		...
		if ($con1) echo "<a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksOrder','weight',$weight,'bidori',$bid,'weightrep',$weight_minus,'bidrep',$con1)."\"><img src=\"pic/admin/up.gif\" alt=\"Переместить вверх\" title=\"Переместить вверх\" border=\"0\"></a> ";
		if ($con2) echo "<a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksOrder','weight',$weight,'bidori',$bid,'weightrep',$weight_plus,'bidrep',$con2)."\"><img src=\"pic/admin/down.gif\" alt=\"Переместить вниз\" title=\"Переместить вниз\" border=\"0\"></a>";
		echo"</td>";
		if ($bkey == "") {
			$type = "HTML";
		...
		echo "<td align=\"center\"><a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksEdit','bid',$bid)."\" title=\"Редактировать\"><img src=\"pic/admin/edit.gif\" border=\"0\" alt=\"Редактировать\"></a> <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksChange','bid',$bid)."\" $change";
		if ($bkey == "") echo " <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksDelete','bid',$bid)."\" OnClick=\"return DelCheck(this, 'Удалить &quot;$title&quot;?');\" title=\"Удалить\"><img src=\"pic/admin/delete.gif\" border=\"0\" alt=\"Удалить\"></a>";
		if ($block_act == 0) echo " <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksShow','bid',$bid)."\" title=\"Показать\"><img src=\"pic/admin/show.gif\" border=\"0\" alt=\"Показать\"></a>";
	}
...
function BlocksNew() {
	...
	."<form action=\"".$REL_SEO->make_link('blocksadmin')."\" method=\"post\">"
...
function BlocksOrder($weightrep,$weight,$bidrep,$bidori) {
	...
	$REL_CACHE->clearGroupCache('blocks');
	safe_redirect($REL_SEO->make_link('blocksadmin'));
}
...
function BlocksShow($bid) {
	...
	echo "<h4>[ <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksChange','bid',$bid)."\">Включить</a> | <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksEdit','bid',$bid)."\">Редактировать</a>";
	if ($bkey == "") echo " | <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksDelete','bid',$bid)."\" OnClick=\"return DelCheck(this, 'Удалить &quot;$title&quot;?');\">Удалить</a>";
	echo " | <a href=\"".$REL_SEO->make_link('blocksadmin')."\">Главная</a> ]</h4>";
}
...
function BlocksChange($bid, $ok=0) {
	...
	if (($ok) || ($active == 0)) {
		...
		if ($active != 0) stderr("<center>Деактивировать блок \"$title\"?<br /><br />","[ <a href=\"".$REL_SEO->make_link('blocksadmin','op','BlocksChange','bid',$bid,'ok',1)."\">Да</a> | <a href=\"".$REL_SEO->make_link('blocksadmin')."\">Нет</a> ]</center>",'success');
	}
}
...
switch($op) {
	...
	case "BlocksDelete":
		if (!is_valid_id($_GET['bid'])) stderr($REL_LANG->say_by_key('error'),$REL_LANG->say_by_key('invalid_id'));
