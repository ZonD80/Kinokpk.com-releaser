...
function process_values()
{
    ...
    foreach ($fields as $field) {
        if (in_array($field, $fields_to_strip)) {
            if (!isset($_POST[$field])) $REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('no_value') . $REL_LANG->say_by_key('to_rgadmin'));
...
if ($id && (!is_valid_id($id) || !@mysql_result($REL_DB->query("SELECT 1 FROM relgroups WHERE id = $id"), 0))) $REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('invalid_id'));


if (!$a) {

    $REL_TPL->stdhead($REL_LANG->say_by_key('rg_title'));
    $res = $REL_DB->query("SELECT relgroups.*, COUNT(rg_subscribes.id) AS users FROM relgroups LEFT JOIN rg_subscribes ON relgroups.id=rg_subscribes.rgid GROUP BY relgroups.id ORDER BY relgroups.added DESC");
    ...
    if (!$rgarray) {
        print '<tr><td colspan="14">' . $REL_LANG->say_by_key('no_relgroups') . '</td></tr>';
        $REL_TPL->end_frame();
        $REL_TPL->stdfoot();
        die();
    }
    ...
    $ownres = $REL_DB->query("SELECT id,username,class, warned, donor, enabled FROM users WHERE id IN(" . implode(',', ($memarray ? array_merge($uidsarray, $memarray) : $uidsarray)) . ")");
    ...
    $REL_TPL->stdhead($REL_LANG->say_by_key('rg_title'));
    if ($a == 'edit') {
        $groupsql = $REL_DB->query("SELECT * FROM relgroups WHERE id=$id");
...
elseif ($a == 'saveadd') {
    ...
    if (mysql_errno()) $REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('group_error') . $REL_LANG->say_by_key('to_rgadmin'));
        ...
        $REL_TPL->stderr($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('group_added'), 'success');
    }
...
elseif ($a == 'saveedit') {
    ...
    $REL_DB->query("UPDATE relgroups SET $string WHERE id = $id");
    safe_redirect($REL_SEO->make_link('relgroups', 'id', $id), 3);
    $REL_TPL->stderr($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('group_edited'), 'success');
}
elseif ($a == 'deleteusers') {
    $REL_DB->query("DELETE FROM rg_subscribes WHERE rgid = $id");
    safe_redirect($REL_SEO->make_link('rgadmin'), 3);
    $REL_TPL->stderr($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('users_deleted'), 'success');
}
elseif ($a == 'users') {
    $REL_TPL->stdhead($REL_LANG->say_by_key('view_users'));
    $count = get_row_count('rg_subscribes', "WHERE rgid=$id");

    $res = $REL_DB->query("SELECT rg_subscribes.*, users.username, users.class, users.warned, users.donor, users.enabled FROM rg_subscribes LEFT JOIN users ON rg_subscribes.userid=users.id WHERE rgid = $id ORDER BY valid_until ASC $LIMIT");
    $REL_TPL->begin_frame($REL_LANG->say_by_key('view_users') . ' ' . @mysql_result($REL_DB->query("SELECT name FROM relgroups WHERE id = $id"), 0) . $REL_LANG->say_by_key('to_rgadmin'));
    begin_table();
...
elseif ($a == 'deleteuser') {
    ...
    $groupsql = $REL_DB->query("SELECT name FROM relgroups WHERE id=$id");
    $group = mysql_fetch_assoc($groupsql);
    if (!$userid) $REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('invalid_id'));
    $notify = isset($_GET['notify']);
    $REL_DB->query("DELETE FROM rg_subscribes WHERE userid=$userid AND rgid=$id");
    ...
    $REL_TPL->stderr($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('delete_user_ok') . ($notify ? " {$REL_LANG->say_by_key('notify_send')}" : ''), 'success');

}
elseif ($a == 'delete') {
    $REL_DB->query("DELETE FROM relgroups WHERE id=$id");
    $REL_DB->query("UPDATE torrents SET relgroup=0 WHERE relgroup=$id");
    $REL_DB->query("DELETE FROM rg_subscribes WHERE rgid=$id");
    $REL_DB->query("DELETE FROM comments WHERE toid=$id AND type='rg'");
    $newstodeletesql = $REL_DB->query("SELECT comments.id FROM comments LEFT JOIN rgnews ON comments.toid=rgnews.id WHERE rgnews.relgroup=$id AND comments.type='rgnews'");
    while (list($nid) = mysql_fetch_array($newstodeletesql)) $nids[] = $id;
    $REL_DB->query("DELETE FROM rgnews WHERE relgroup=$id");
    if ($nids) $REL_DB->query("DELETE FROM comments WHERE toid IN (" . implode(',', $nids) . ") AND type='rgnews'");
    safe_redirect($REL_SEO->make_link('rgadmin'), 1);
    $REL_TPL->stderr($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('relgroup_deleted'), 'success');
}
else $REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('unknown_action'));
