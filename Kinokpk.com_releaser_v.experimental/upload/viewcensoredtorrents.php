...
if (is_array($_GET["delt"]))
{
	if (get_privilege('censored_admin',false)) {
		if (empty($_GET["delt"]))
		$REL_TPL->stderr($REL_LANG->say_by_key('error'),$REL_LANG->say_by_key('no_fields_blank'));
		$REL_DB->query("DELETE FROM censoredtorrents WHERE id IN (" . implode(", ", array_map("sqlesc", $_GET["delt"])) . ")");

		$REL_CACHE->clearGroupCache("block-cen");
		$REL_TPL->stderr($REL_LANG->say_by_key('success'), $REL_LANG->say_by_key('ban_uninstalled'));
		$REL_TPL->stdfoot();
		die;
	}
	else  {
		$REL_TPL->stderr($REL_LANG->say_by_key('error'), $REL_LANG->say_by_key('not_permission_prohibitions'));
		$REL_TPL->stdfoot();
		die;
	}
}
...
$res = $REL_DB->query("SELECT * FROM censoredtorrents ORDER BY id DESC");
print ("<tr>");
