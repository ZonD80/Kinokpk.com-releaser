...
<form action="<?php print $REL_SEO->make_link('langadmin', 'import');?>"
    ...
    <table>
        <tr>
            <td><input type="text" name="language" maxlength="2"
                       size="2"/> <?php print $REL_LANG->_("Language to be imported, e.g. 'ru,en,ua'");?></td>
            <td><input type="file" name="langfile"/> <?php print $REL_LANG->_("Language file as it is");?></td>
            <td><input type="checkbox" name="override" value="1"/> <?php print $REL_LANG->_("Override current data");?>
            </td>
            <td><input type="submit"
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if ($_GET['a'] == 'saveadd') {
            ...
            foreach ($lang as $l => $word) {
                ...
                if (mysql_errno() == 1062) {
                    $REL_TPL->stdmsg($REL_LANG->_("Error"), 'REDECLARATED KEY:"' . $key . '"', 'error');
                    $REL_TPL->stdfoot();
                    die();
                }
...
<table>
    <tr>
        <td><input type="hidden" name="editor" value="1"/> <?php print $REL_LANG->_("Search by key or value");?>
            <?php print $REL_LANG->_('Key');?>:<input type="text" name="searchkey" value="<?php print $searchkey;?>"/>&nbsp;<?php print $REL_LANG->_('Value');?>
            :<input type="text" name="searchvalue" value="<?php print $searchvalue;?>"/></td>
        <td><input type="submit"
                   value="<?php print $REL_LANG->_("Continue");?>"/></td>
    </tr>
</table>
