...
if(empty($_FILES['avatar']['tmp_name'])) {
	...
	print "<br /><form method=post action=\"avatarup.php\" enctype=\"multipart/form-data\">".$tracker_lang['select_avatar']." : <input type=\"file\" name=\"avatar\">
<input type=submit value=".$tracker_lang['upload']." ></form><br /><br /><center><font color=green>(".sprintf($tracker_lang['hint'], number_format(round($maxfilesize/1024,2)), number_format($max_image_width), number_format($max_image_height)).")</font></center>";
	...
	}else
	{
      ...
      copy($_FILES['avatar']['tmp_name'],$pathav);		sql_query("UPDATE users SET avatar = '".$pathav."' WHERE id = " . $CURUSER["id"])or sqlerr(__FILE__,__LINE__);
		...
		stderr($tracker_lang['success'],$tracker_lang['succes_upload']." ".$CURUSER["id"].substr($_FILES['avatar']['name'], strripos($_FILES['avatar']['name'], '.'))."".sprintf($tracker_lang['file_size'], number_format(round($_FILES['avatar']['size']/1024,2))),'success');
	}
}
?>