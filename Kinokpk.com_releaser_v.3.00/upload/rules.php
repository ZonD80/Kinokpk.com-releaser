...
if (isset($rules_categ)) {
	...
	begin_frame("<a id=\"top\"/>Содержание");
	print("<ul>");
	foreach ($rules_categ as $id => $temp) {
		if ($rules_categ[$id][flag] == "1") {
			...
			/*---------------------------------------------------------------------
			 ...
			 print("<li><a href=\"rules.php#". $id2 ."\" class=\"altlink\">". $rules_categ[$id][items][$id2][rule] ."</a> <img src=\"pic/updated.png\" alt=\"Обновлено\" title=\"Обновлено\" align=\"absbottom\"></li>\n");
			 elseif ($rules_categ[$id][items][$id2][flag] == "3")
			 print("<li><a href=\"rules.php#". $id2 ."\" class=\"altlink\">". $rules_categ[$id][items][$id2][rule] ."</a> <img src=\"pic/new.png\" alt=\"Новое\" title=\"Новое\" align=\"absbottom\"></li>\n");
			 }
			 }
			 ---------------------------------------------------------------------*/
		}
	...
	foreach ($rules_categ as $id => $temp) {
		if ($rules_categ[$id][flag] == "1") {
			$frame = $rules_categ[$id][title] ." - <a href=\"rules.php#top\">Наверх</a>";
			begin_frame($frame);
			print("<a id=\"n". $id ."\"/>\n<ul>\n");
			if (array_key_exists("items", $rules_categ[$id])) {
				foreach ($rules_categ[$id][items] as $id2 => $temp) {
