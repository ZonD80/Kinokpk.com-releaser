...
if (!isset($_GET['action'])) {
	stdhead("FAQ Administration / Админка ЧаВо");
	...
	if (isset($faq_categ)) {
		...
		// print the faq table
		print("<form method=\"post\" action=\"faqadmin.php?action=reorder\">");
		...
		foreach ($faq_categ as $id => $temp) {
			...
			print("</select></td><td align=\"center\" width=\"40px\">&nbsp;</td><td><b>". $faq_categ[$id]["title"] ."</b></td><td align=\"center\" width=\"60px\">". $status ."</td><td align=\"center\" width=\"60px\"><a href=\"faqadmin.php?action=edit&id=". $id ."\">E</a> / <a onClick=\"return confirm('Вы уверены?')\" href=\"faqadmin.php?action=delete&id=". $id ."\">D</a></td></tr>\n");
			...
			if (array_key_exists("items", $faq_categ[$id])) {
				foreach ($faq_categ[$id]["items"] as $id2 => $temp) {
					...
					elseif ($faq_categ[$id]["items"][$id2][flag] == "2") $status = "<font color=\"#0000FF\"><img src=\"".ROOT_PATH."pic/updated.png\" alt=\"Updated\" align=\"absbottom\"></font>";
					elseif ($faq_categ[$id]["items"][$id2][flag] == "3") $status = "<font color=\"#008000\"><img src=\"".ROOT_PATH."pic/new.png\" alt=\"Новое\" align=\"absbottom\"></font>";
					else $status = "Обычный";
					print("</select></td><td>". $faq_categ[$id]["items"][$id2]["question"] ."</td><td align=\"center\" width=\"60px\">". $status ."</td><td align=\"center\" width=\"60px\"><a href=\"faqadmin.php?action=edit&id=". $id2 ."\">E</a> / <a onClick=\"return confirm('Вы уверены?')\" href=\"faqadmin.php?action=delete&id=". $id2 ."\">D</a></td></tr>\n");
				}
			}

			print("<tr><td colspan=\"5\" align=\"center\"><a href=\"faqadmin.php?action=additem&inid=". $id ."\">Добавить новый элемент</a></td></tr>\n");
			print("</table>\n");
		}
	}
	...
	if (isset($faq_orphaned)) {
		...
		foreach ($faq_orphaned as $id => $temp) {
			...
			print("<tr><td>". $faq_orphaned[$id]["question"] ."</td><td align=\"center\" width=\"60px\">". $status ."</td><td align=\"center\" width=\"60px\"><a href=\"faqadmin.php?action=edit&id=". $id ."\">edit</a> <a onClick=\"return confirm('Вы уверены?')\" href=\"faqadmin.php?action=delete&id=". $id ."\">delete</a></td></tr>\n");
		}
		print("</table>\n");
	}

	print("<br />\n<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" align=\"center\" width=\"95%\">\n<tr><td align=\"center\"><a href=\"faqadmin.php?action=addsection\">Добавить новую секцию</a></td></tr>\n</table>\n");
...
elseif ($_GET["action"] == "reorder") {
	...
	safe_redirect(" faqadmin.php");
}

// ACTION: edit - edit a section or item
elseif ($_GET["action"] == "edit" && isset($_GET[id])) {
	stdhead("FAQ Administration / Админка ЧаВо");
	print("<h2>Редактировать ЧаВо</h2>");
	...
	while ($arr = mysql_fetch_array($res, MYSQL_BOTH)) {
		...
		if ($arr[type] == "item") {
			mcejs();
			print("<form method=\"post\" action=\"faqadmin.php?action=edititem\">");
			...
			print("<tr><td style=\"vertical-align: top;\">Ответ:</td><td><textarea id=tmce rows=15 cols=80 name=\"answer\">$arr[answer]</textarea></td></tr>\n");
		...
		elseif ($arr[type] == "categ") {
			print("<form method=\"post\" action=\"faqadmin.php?action=editsect\">");
...
elseif ($_GET["action"] == "delete" && isset($_GET["id"])) {
	...
	elseif ($_GET[action] == "additem" && $_GET["inid"]) {
		...
		print("<form method=\"post\" action=\"faqadmin.php?action=addnewitem\">");
		print("<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" align=\"center\" width=\"100%\">\n");
		print("<tr><td>Вопрос:</td><td><input id=specialboxg type=\"text\" name=\"question\" value=\"\" /></td></tr>\n");
		print("<tr><td style=\"vertical-align: top;\">Ответ:</td><td><textarea id=tmce rows=15 cols=80 name=\"answer\"></textarea></td></tr>\n");
	...
	elseif ($_GET[action] == "addsection") {
		...
		print("<form method=\"post\" action=\"faqadmin.php?action=addnewsect\">");
		print("<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" align=\"center\" width=\"100%\">\n");
	...
	else safe_redirect(" faqadmin.php");
