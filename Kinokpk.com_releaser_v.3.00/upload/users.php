...
if ((get_user_class () >= UC_MODERATOR) && $_GET['act']) {
	if ($_GET['act'] == "users") {
		...
		while ($row = mysql_fetch_array($result)) {
			...
			echo "<tr><td><a href=userdetails.php?id=".$row["id"]."><b>".$row["username"]."</b></a></td><td><strong>".$ratio."</strong></td><td>".$row["ip"]."</td><td>".mkprettytime($row["added"])."</td><td>".mkprettytime($row["last_access"])." (".get_elapsed_time($row["last_access"],false)." {$tracker_lang['ago']})</td>"/*<td>".(int)$row['downloaded']."</td><td>".(int)$row['seeding']."</td>*/."</tr>";


		}
		if (!$records) stdmsg($tracker_lang['error'],$tracker_lang['nothing_found'],'error');
...
elseif (!isset($_GET['act'])) {
	...
	print("</select>\n");
	print("<input type=\"submit\" class=\"button\" style=\"margin-top:-5px\" value=\"{$tracker_lang['go']}\">\n");
	print("</form>\n");
	print("</div\n");
...
"<td>".mkprettytime($arr['added'])."</td><td>".mkprettytime($arr['last_access'])." (".get_elapsed_time($arr["last_access"],false)." {$tracker_lang['ago']})</td><td>$ratio</td><td>$gender</td>".
