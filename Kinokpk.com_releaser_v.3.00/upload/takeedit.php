...
EOD;
	...
	if (!isset($_POST['trackers'])) stderr($tracker_lang['error'],'Не все поля заполнены');
	$POSTtrackers = explode("\n",trim((string)$_POST['trackers']));
	if (!$POSTtrackers) stderr($tracker_lang['error'], 'Ошибка обработки трекеров');
	...
	stdfoot();
	write_log("<a href=\"userdetails.php?id=\"{$CURUSER['id']}>{$CURUSER['username']}</a> отредактировал трекера торрента с ID <a href=\"details.php?id=$id\">$id</a>",'torrent');
	die();
}
...
stderr($tracker_lang['success'],"Релиз успешно обновлен, сейчас вы перейдете к его деталям".($anarray?"<img src=\"remote_check.php?id=$id\" width=\"0px\" height=\"0px\" border=\"0\"/>":''),'success');
