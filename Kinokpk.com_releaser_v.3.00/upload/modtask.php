...
if (($action == 'ownsupport') && (get_user_class()>=UC_ADMINISTRATOR)) {
	...
	safe_redirect("my.php",0);
	stderr($tracker_lang['success'],'Вы успешно сменили себе статус поддержки','success');

}
elseif ($action == "edituser") {
	$userid = (int) $_POST["userid"];
	$CLASS = @mysql_result(sql_query("SELECT class FROM users WHERE id = $userid"),0);
	if ($CLASS >= get_user_class()) stderr($tracker_lang['error'],$tracker_lang['access_denied']);
	...
	write_log("Пользователь {$CURUSER['username']} произвел действия над пользователем с ID <a href=\"userdetails.php?id=$userid\">$userid</a>, параметры:<br/> <pre>".var_export($updateset,true)."</pre>",'modtask');
