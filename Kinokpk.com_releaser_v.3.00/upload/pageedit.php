...
if (($CURUSER["id"] != $row["owner"]) && (get_user_class() < UC_MODERATOR)) {
	stdmsg($tracker_lang['error'],"Вы не можете редактировать эту страницы.");
} else {
	...
	if (get_user_class() >= UC_MODERATOR) {
		...
		tr("Создатель - ".$tracker_lang['from_system'],"<input type=\"checkbox\" name=\"system\" value=\"1\"".((!$row['owner'])?" checked=\"1\"":'').">",1);
