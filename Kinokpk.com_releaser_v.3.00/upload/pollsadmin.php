...
elseif ($_GET['action'] == 'add') {
	...
	print('<form name="add" action="pollsadmin.php?action=add2" method="post"><table width="100%" border="1">
...
elseif (($_GET['action'] == 'saveadd') && ($_SERVER['REQUEST_METHOD'] == 'POST')) {
	...
	safe_redirect(" polloverview.php?id=$pollid");
}
...
elseif (($_GET['action'] == 'saveedit') && ($_SERVER['REQUEST_METHOD'] == 'POST')) {
	...
	$CACHE->clearGroupCache("block-polls");
	safe_redirect(" polloverview.php?id=$id");
}
