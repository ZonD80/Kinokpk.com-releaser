...
$newsid = (int) $_GET['id'];
if (!is_valid_id($newsid)) 			stderr($tracker_lang['error'], $tracker_lang['invalid_id']);
//$action = $_GET["action"];
...
if (isset($_GET['id'])) {
	...
	if (!$news) stderr($tracker_lang['error'],$tracker_lang['invalid_id']);

	$added = mkprettytime($news['added']) . " (" . (get_elapsed_time($news["added"],false)) . " {$tracker_lang['ago']})";
	print("<h1>{$news['subject']}</h1>\n");
	print("<table width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"5\">\n" .
 "<tr><td class=\"colhead\">Содержание&nbsp;<a href=newsoverview.php?id=$newsid#comments>Комментировать</a></td></tr>\n");
	...
	if (!$count) {
		...
		print("<div align=\"right\"><a href=newsoverview.php?id=$newsid#comments class=altlink_white>Добавить комментарий</a></div>");
		print("</td></tr><tr><td align=\"center\">");
		print("Комментариев нет. <a href=newsoverview.php?id=$newsid#comments>Желаете добавить?</a>");
		...
		while ($subrow = mysql_fetch_array($subres)) {
			...
			$subrow['link'] = "newsoverview.php?id=$newsid#comm{$subrow['id']}";
			$allrows[] = $subrow;
		}
	...
	print ( "<form name=comment method=\"post\" action=\"newscomment.php?action=add\">" );
