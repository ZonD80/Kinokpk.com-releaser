...
$CACHEARRAYrow = sql_query("SELECT * FROM cache_stats WHERE cache_name IN ('sitename','defaultbaseurl','siteemail','default_language','use_lang','smtptype')");

while ($CACHEARRAYres = mysql_fetch_assoc($CACHEARRAYrow)) $CACHEARRAY[$CACHEARRAYres['cache_name']] = $CACHEARRAYres['cache_value'];
//$CACHEARRAY['defaultbaseurl'] = mysql_result(sql_query("SELECT cache_value FROM cache_stats WHERE cache_name='defaultbaseurl'"),0);
...
while ($message = mysql_fetch_assoc($emails)) {
	sent_mail($message['email'], $message['subject'].' | '.$CACHEARRAY['sitename'], $CACHEARRAY['siteemail'], $message['subject'], $message['body']) or write_log("Sending mail to {$message['email']} <font color=\"red\">FAILED</font>",'emailnotifs_cron');
}
