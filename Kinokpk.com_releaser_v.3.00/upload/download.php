...
if ($rating_enabled && ($CURUSER['ratingsum']<$CRON['rating_downlimit'])) stderr($tracker_lang['error'],$tracker_lang['rating_low']);

$currating = $CURUSER['ratingsum']-$CRON['rating_perdownload'];
if ($currating>0) $znak='+';
if (!$readed) stderr($tracker_lang['downloading_torrent'],($rating_enabled?sprintf($tracker_lang['download_notice'],$CRON['rating_perdownload'],$znak.$currating,$CRON['rating_downlimit']).'<br />':'').'<div align="center"><form action="download.php"><input type="hidden" name="id" value="'.$id.'"><input type="hidden" name="ok" value=""><input type="submit" value="'.$tracker_lang['download_torrent'].'">&nbsp;<input type="submit" name="magnet" value="'.$tracker_lang['as_magnet'].'"></form></div>'.(!REL_AJAX?sprintf($tracker_lang['to_details'],$id):''),'success');
...
if (!$magnet && (@ini_get('output_handler') == 'ob_gzhandler') && (@ob_get_length() !== false))
...
if ($magnet) {
	...
	$message = $tracker_lang['this_is_magnet'].'<h1><a href="'.$link.'">'.$tracker_lang['magnet'].'</a></h1>'.(!REL_AJAX?sprintf($tracker_lang['to_details'],$id):'');
	stderr($tracker_lang['this_is_magnet_title'],$message,'success');
}
...
header ("Content-Disposition: attachment; filename=\"{$_SERVER['HTTP_HOST']}-$id-".$row['filename']."\"");
