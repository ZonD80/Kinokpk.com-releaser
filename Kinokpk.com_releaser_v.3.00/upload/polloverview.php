...
if (isset($_GET['vote'])  && ($_SERVER['REQUEST_METHOD'] == 'POST')){
	if ((isset($_POST["vote"]) && !is_valid_id($_POST["vote"])) || !is_valid_id($pid)) 			stderr($tracker_lang['error'], $tracker_lang['invalid_id']);
	...
	$CACHE->clearGroupCache("block-polls");
	safe_redirect(" polloverview.php?id=$pid");

}

if (!is_valid_id($pid)) 			stderr($tracker_lang['error'], $tracker_lang['invalid_id']);
...
print '<div id="polls" width="100%" border="1">
		...
		<ul class="polls_title_q">
			<li align="center" class="colheadli" colspan="2"><h3 style="margin-top: 7px;margin-bottom:0;">'.$pquestion.'</h3>'.((get_user_class() >= UC_ADMINISTRATOR)?" <span style=\"margin-left: 335px;\">[<a href=\"pollsadmin.php?action=add\">Создать новый</a>] [<a href=\"pollsadmin.php?action=edit&id=$id\">Редактировать</a>] [<a onClick=\"return confirm('Вы уверены?')\" href=\"pollsadmin.php?action=delete&id=$id\">Удалить</a>]":"<span>").'</li>
		</ul>';
...
foreach ($sidcount as $sidkey => $vsid){
	...
	elseif (((!is_null($pexp) && ($pexp > time())) || is_null($pexp)) && !$voted) print "<form name=\"voteform\" method=\"post\" action=\"polloverview.php?vote&id=$id\"><ul><dt class=\"polls_right\">
...
elseif ($voted) print'<ul><li><span style="color:red;">Вы уже голосовали в этом опросе</span></li>';
print'<li align="center">Всего голосов: '.$tvotes.', Комментариев: '.$comments.' [<a href="polloverview.php?id='.$id.'"><b>Подробнее</b></a>] [<a href="polloverview.php?id='.$id.'#comments"><b>Комментировать</b></a>] [<a href="pollsarchive.php"><b>Архив опросов</b></a>]</li></ul>'.($novote?'</form>':'');
...
if (!$count) {
	...
	print("<div align=\"right\"><a href=polloverview.php?id=$pid#comments class=altlink_white>{$tracker_lang['add_comment']}</a></div>");
	print("</td></tr><tr><td align=\"center\">");
	print("Комментариев нет. <a href=polloverview.php?id=$pid#comments>Желаете добавить?</a>");
	print("</td></tr></table><br />");

}
else {
	list($pagertop, $pagerbottom, $limit) = pager($limited, $count, "polloverview.php?id=".$_GET['id']."&", array(lastpagedefault => 1));
	...
	while ($subrow = mysql_fetch_array($subres)) {
		...
		$subrow['link'] = "polloverview.php?id=$id#comm{$subrow['id']}";
		$allrows[] = $subrow;
	}
	...
	print("<div align=\"right\"><a href=polloverview.php?id=$pid#comments class=altlink_white>Добавить комментарий</a></div>");
	print("</td></tr>");
