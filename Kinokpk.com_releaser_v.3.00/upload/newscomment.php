...
if ($action == "add")
{
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if (!$nsubject) stderr($tracker_lang["error"],$tracker_lang["invalid_id"]);
		...
		if ($last_pmrow[0]){
			...
			if ($CACHEARRAY['as_check_messages'] && ($last_pmrow[0]['msg'] == $text) && ($last_pmrow[1]['msg'] == $text) && ($last_pmrow[2]['msg'] == $text) && ($last_pmrow[3]['msg'] == $text)) {
				...
				foreach ($msgids as $key => $msgid){
					$msgview.= "\n<a href=newsoverview.php?id={$torids[$key]}#comm$msgid>Комментарий ID={$msgid}</a> от пользователя ".$CURUSER['username'];
				}
				...
				if (strpos($modcomment,"Maybe spammer in news") === false) {
					...
					while (list($admin) = mysql_fetch_array($arow)) {
						...
						$admin, '" . time() . "', 'Пользователь <a href=userdetails.php?id=".$CURUSER['id'].">".$CURUSER['username']."</a> может быть спамером, т.к. его 5 последних посланных комментариев к новостям полностью совпадают.$msgview', 'Сообщение о спаме!', 1)") or sqlerr(__FILE__, __LINE__);
					}
					...
					while (list($admin) = mysql_fetch_array($arow)) {
						sql_query("INSERT INTO messages (poster, sender, receiver, added, msg, subject, location) VALUES(0, 0,
						$admin, '" . time() . "', 'Пользователь <a href=userdetails.php?id=".$CURUSER['id'].">".$CURUSER['username']."</a> забанен системой за спам в комментариях к новостям, его IP адрес (".$CURUSER['ip'].")', 'Сообщение о спаме [бан]!', 1)") or sqlerr(__FILE__, __LINE__);
		...
		send_comment_notifs($nid,"<a href=\"newsoverview.php?id=$nid#comm$newid\">{$nsubject}</a>",'newscomments');
		safe_redirect(" newsoverview.php?id=$nid#comm$newid");
		die;
	}
}
elseif ($action == "quote")
