...
if(!is_valid_id($nid) && !is_valid_id($commentid)) stderr($tracker_lang["error"],$tracker_lang["invalid_id"]);
...
if ($relgroup['private']) {
	if (!in_array($rgid,@explode(',',$CURUSER['relgroups'])) && !$I_OWNER && !in_array($CURUSER['id'],@explode(',',$relgroup['onwers']))) stderr($tracker_lang['error'],$tracker_lang['no_access_priv_rg']);
}
...
if ($action == "add")
{
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if ($last_pmrow[0]){
			...
			if ($CACHEARRAY['as_check_messages'] && ($last_pmrow[0]['msg'] == $text) && ($last_pmrow[1]['msg'] == $text) && ($last_pmrow[2]['msg'] == $text) && ($last_pmrow[3]['msg'] == $text)) {
				...
				foreach ($msgids as $key => $msgid){
					$msgview.= "\n<a href=rgnewsoverview.php?id={$torids[$key]}#comm$msgid>Комментарий ID={$msgid}</a> от пользователя ".$CURUSER['username'];
				}
				...
				if (strpos($modcomment,"Maybe spammer in rgnewscomments") === false) {
					...
					while (list($admin) = mysql_fetch_array($arow)) {
						...
						$admin, '" . time() . "', 'Пользователь <a href=userdetails.php?id=".$CURUSER['id'].">".$CURUSER['username']."</a> может быть спамером, т.к. его 5 последних посланных комментариев к новости релиз-группы полностью совпадают.$msgview', 'Сообщение о спаме!', 1)") or sqlerr(__FILE__, __LINE__);
					}
					...
					while (list($admin) = mysql_fetch_array($arow)) {
						sql_query("INSERT INTO messages (poster, sender, receiver, added, msg, subject, location) VALUES(0, 0,
						$admin, '" . time() . "', 'Пользователь <a href=userdetails.php?id=".$CURUSER['id'].">".$CURUSER['username']."</a> забанен системой за спам в комментариях к новостям релиз-групп, его IP адрес (".$CURUSER['ip'].")', 'Сообщение о спаме [бан]!', 1)") or sqlerr(__FILE__, __LINE__);
						stderr("Поздравляем!","Вы успешно забанены системой за спам в комментариях к новостям релиз-групп! Если вы не согласны с решением системы, <a href=\"contact.php\">подайте жалобу админам</a>.");
					}
				}
				stderr($tracker_lang['error'],"На нашем сайте стоит защита от спама, ваши 5 последних комментариев к новостям релиз-групп совпадают. В отсылке комментария отказано. <b><u>ВНИМАНИЕ! ЕСЛИ ВЫ ЕЩЕ РАЗ ПОПЫТАЕТЕСЬ ОТПРАВИТЬ ИДЕНТИЧНОЕ СООБЩЕНИЕ, ВЫ БУДЕТЕ АВТОМАТИЧЕСКИ ЗАБЛОКИРОВАНЫ СИСТЕМОЙ!!!</u></b> <a href=\"javascript: history.go(-1)\">Назад</a>");
		...
		$CACHE->clearCache('relgroups-'.$relgroup['id'], 'newsquery');
			
		safe_redirect(" rgnewsoverview.php?id=$nid#comm$newid");
		die;
	}
}
elseif ($action == "quote")
{
	...
	print("<form method=\"post\" name=\"comment\" action=\"rgnewscomment.php?action=add\">\n");
...
elseif ($action == "edit")
{
	...
	print("<form method=\"post\" name=\"comment\" action=\"rgnewscomment.php?action=edit&amp;cid=$commentid\">\n");
	print("<input type=\"hidden\" name=\"returnto\" value=\"rgnewsoverview.php?id={$arr["nid"]}#comm$commentid\" />\n");
...
elseif ($action == "delete")
{
	...
	$returnto = "rgnewsoverview.php?id=$nid#comm$commentid";
