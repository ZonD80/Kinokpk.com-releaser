...
if (! $user ["confirmed"])
stderr ( $tracker_lang ['error'], "Этот пользователь еще не подтвердил себя по e-mail, возможно, этот аккаунт вскоре будет удален" );
...
elseif ($CURUSER ["id"] != $user ["id"]) {
	...
	if ($friend)
	print ( "<p>(<a href=\"friends.php?action=deny&id=$friend\">Убрать из друзей</a>)<br />(<a href=\"present.php?id=$id\">Подарить подарок</a>)</p>\n" );
	else {
...
print ( "<table style=\"margin-top: 2px;\" cellpadding=\"5\" width=\"100%\">" );
print ( "<tr><td class=colhead align=\"left\" colspan=\"2\">  <div id=\"comments\"></div><b>:: {$tracker_lang['add_comment']} к пользователю | " . is_i_notified ( $id, 'usercomments' ) . "</b></td></tr>" );
...
function togglepic(bu, picid, formid)
{
    ...
    if(pic.src == bu + "/pic/plus.gif")
    {
		...
		print ( "<tr><td class=\"rowhead\">Изменить рейтинг</td><td align=\"left\"><img src=\"pic/plus.gif\" id=\"ratingpic\" onClick=\"togglepic('{$CACHEARRAY['defaultbaseurl']}','ratingpic','ratingchange')\" style=\"cursor: pointer;\">&nbsp;<input type=\"text\" name=\"amountrating\" size=\"10\" /><td>Сейчас рейтинга у пользователя: {$user['ratingsum']}</td></tr>" );
		print ( "<tr><td class=\"rowhead\">Изменить откуп</td><td align=\"left\"><img src=\"pic/plus.gif\" id=\"discountpic\" onClick=\"togglepic('{$CACHEARRAY['defaultbaseurl']}','discountpic','discountchange')\" style=\"cursor: pointer;\">&nbsp;<input type=\"text\" name=\"amountdiscount\" size=\"10\" /><td>Сейчас откупа у пользователя: {$user['discount']}</td></tr>" );
