...
<H1><?=$mailbox_name?></H1>
	...
	$all_mess_procent = round(($all_mess/$CACHEARRAY['pm_max'])*100);
	//print($all_mess_procent);
	$inbox_all_procent = round(($inbox_all/$CACHEARRAY['pm_max'])*100);
	$outbox_all_procent = round(($outbox_all/$CACHEARRAY['pm_max'])*100);

	print("<b>Ваш ящик заполнен на:</b><font color=\"#8da6cf\"> $all_mess</font><font color=\"green\"> ($all_mess_procent%) </font> <small>максимальное количество сообщений - ". $CACHEARRAY['pm_max'] ."</small><br />");
	#amount end

	print('<form id="message" action="message.php" method="POST"><input type="hidden" name="action" value="moveordel">');
	print("<div id=\"tabs\"><ul>
	<li class=\"tab".(($mailbox != PM_SENTBOX)?'1':'2')."\"><a href=\"message.php?action=viewmailbox&box=1\"><span>Входящие <font color=\"#8da6cf\">$inbox_all </font><font color=\"green\">($inbox_all_procent%) </font></span></a></li>
	<li nowrap=\"\" class=\"tab".(($mailbox != PM_SENTBOX)?'2':'1')."\"><a href=\"message.php?action=viewmailbox&box=-1\"><span>Отправленные <font color=\"#8da6cf\">$outbox_all </font><font color=\"green\">($outbox_all_procent%) </font></span></a></li>
	</ul></div>");
	?>
...
<TABLE border="0" cellpadding="4" cellspacing="0" width="100%"
	...
	<TR>
		...
		<TD width="2%" class="colhead"><input id="toggle-all"
			...
			value="<?=$tracker_lang ['mark_all'];?>" /></TD>
	...
	if (mysql_num_rows ( $res ) == 0) {
		...
		while ( $row = mysql_fetch_assoc ( $res ) ) {
			...
			if ($row ['sender'] != 0) {
				...
				if ($friend)
				$username .= "<br /><small>[<a href=friends.php?action=deny&id={$row['fid']}>{$tracker_lang['delete_from_friends']}</a>]" . (! $fconf ? "[<a href=friends.php?action=confirm&id={$row['fid']}>{$tracker_lang['confirm']}</a>]" : '') . "</small>";
				else
	...
	<tr class="colhead">
		...
		<input type="submit" name="markread"
			...
			onClick="return confirm('<?=$tracker_lang ['sure_mark_read'];?>')" />
...
<TABLE width="100%" border="0" cellpadding="4" cellspacing="0">
	...
	<TR>
		...
		print ( "[ <A onClick=\"return confirm('Вы уверены?')\" href=\"message.php?action=deletemessage&id=$pm_id\">Удалить</A> ]$reply [ <A href=\"message.php?action=forward&id=$pm_id\">Переслать</A> ]" . reportarea ( $message ['id'], 'messages' ) );
		?></TD>
	</TR>
</TABLE>
		<?
...
elseif ($action == "sendmessage") {
	...
	if ($replyto) {
		...
		$body .= "<blockquote><p>" . format_comment ( $msga ['msg'] ) . "</p><cite>$usra[username]</cite></blockquote><hr /><br /><br />";
		// Change
...
elseif ($action == 'takemessage') {
	...
	if ($last_pmrow [0]) {
		...
		if ($CACHEARRAY ['as_check_messages'] && ($last_pmrow [0] ['msg'] == $msg) && ($last_pmrow [1] ['msg'] == $msg) && ($last_pmrow [2] ['msg'] == $msg) && ($last_pmrow [3] ['msg'] == $msg)) {
			...
			foreach ( $msgids as $msgid ) {
				$msgview .= "\n<a href=message.php?action=viewmessage&id=$msgid>Сообщение с ID={$msgid}</a> от пользователя " . $CURUSER ['username'];
			}
			...
			if (strpos ( $modcomment, "Maybe spammer" ) === false) {
				...
				while ( list ( $admin ) = mysql_fetch_array ( $arow ) ) {
					...
					$admin, '" . time () . "', 'Пользователь <a href=userdetails.php?id=" . $CURUSER ['id'] . ">" . $CURUSER ['username'] . "</a> может быть спамером, т.к. его 5 последних посланных сообщений полностью совпадают.$msgview', 'Сообщение о спаме!', 1)" ) or sqlerr ( __FILE__, __LINE__ );
				}
				$modcomment .= "\n" . time () . " - Maybe spammer";
	...
	if ($pms >= $CACHEARRAY ['pm_max'])
	stderr ( $tracker_lang ['error'], "Ящик личных сообщений получателя заполнен, вы не можете отправить ему сообщение." );

	if ($save) {
		$pms = sql_query ( "SELECT SUM(1) FROM messages WHERE (receiver = " . $CURUSER ['id'] . " AND location=1) OR (sender = " . $CURUSER ['id'] . " AND saved = 1)" );
		$pms = (int)mysql_result ( $pms, 0 );
		if ($pms >= $CACHEARRAY ['pm_max'])
		stderr ( "Невозможно сохранить сообщение", "Ваш ящик личных сообщений заполнен, максимальное кол-во {$CACHEARRAY['pm_max']}. Вы не можете отправить сообщение, вам необходимо очистить ящик личных сообщений" );
	}
	...
	if ($comment || $snapshot) {
		...
		if (mysql_num_rows ( $res ) > 0) {
			...
			while ( $user = mysql_fetch_array ( $res ) ) {
				...
				$new = $comment;
				if ($snapshot) {
					$new .= ($new ? "\n" : "") . "MMed, " . date ( "Y-m-d" ) . ", " . "UL: " . mksize ( $user ['uploaded'] ) . ", " . "DL: " . mksize ( $user ['downloaded'] ) . ", " . "r: " . (($user ['downloaded'] > 0) ? ($user ['uploaded'] / $user ['downloaded']) : 0) . " - " . ($_POST ['sender'] == "system" ? "System" : $CURUSER ['username']);
				}
				$new .= $old ? ("\n" . $old) : $old;
...
elseif ($action == "moveordel") {
	...
	if ($_POST ['move']) {
		...
		safe_redirect(" message.php?action=viewmailbox&box=" . $pm_box );
		exit ();
	} elseif ($_POST ['delete']) {
		if ($pm_id) {
		...
		safe_redirect(" message.php?action=viewmailbox&box=" . $pm_box,1 );
		stderr($tracker_lang['success'], "Сообщение(я) архивировано(ы)!",'success');
		...
		safe_redirect(" message.php?action=viewmailbox&box=" . $pm_box,1 );
		stderr($tracker_lang['success'], "Сообщение(я) разархивировано(ы)!",'success');
	}
...
elseif ($action == "forward") {
	if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
		...
		$body = "Оригинальное сообщение:<hr /><blockquote><p>" . format_comment ( $message ['msg'] . "</p><cite>{$from2['username']}</cite></blockquote><hr /><br /><br />" );
...
<TABLE border="0" cellpadding="4" cellspacing="0">
	...
	<TR>
		...
		$body .= "Оригинальное сообщение:<hr /><blockquote><p>" . $message ['msg'] . "</p><cite>{$from}</cite></blockquote><hr /><br /><br />";
		$save = ( int ) $_POST ['save'];
		if ($save) {
			$save = 1;
		} else {
		...
		if ($pms >= $CACHEARRAY ['pm_max'])
		stderr ( $tracker_lang ['error'], "Ящик личных сообщений получателя заполнен, вы не можете переслать ему сообщение." );
		...
		stderr ($tracker_lang['success'], "ЛС переслано.", "success");
	}
} //конец пересылка
...
elseif ($action == "deletemessage") {
	...
	if (mysql_affected_rows () == 0) {
		...
		safe_redirect(" message.php?action=viewmailbox&id=" . $message ['location'] );
		exit ();
	}
