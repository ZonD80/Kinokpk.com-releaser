...
{
	print "<p align=center>(<a href='usersearch.php?h=1'>Инструкиця</a>)";
	print "&nbsp;-&nbsp;(<a href='usersearch.php'>Сброс</a>)</p>\n";
}
...
?>
	...
	if (count($_GET) > 0 && !$_GET['h'])
	{
	  ...
	  $q .= ($q ? "&amp;" : "") . "co=".urlencode(trim($_GET['co']));
...

			{
...

				{
...

					{
						...
						stdmsg($tracker_lang['error'], "Второй рейтинг должен быть больше первого.");
		...
		$select_is = "u.id, u.username, u.ratingsum, u.email, u.confirmed, u.added, last_access, u.ip,
  	u.class, u.donor, u.modcomment, u.enabled, u.warned";
        ...
        "<td class=colhead align=left>Включен</td>";
          ...
          <td><div align=center>" . mkprettytime($user['last_access']) . " (".get_elapsed_time($user['last_access'])." {$tracker_lang['ago']})</div></td>
          <td><div align=center>" . ($user['confirmed']?$tracker_lang['yes']:$tracker_lang['no']) . "</div></td>
          <td><div align=center>" . ($user['enabled']?$tracker_lang['yes']:$tracker_lang['no'])."</div></td>
			</tr>\n";
			}
			print "</table>";
			if ($count > $perpage)
