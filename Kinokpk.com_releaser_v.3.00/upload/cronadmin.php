...
if (!isset($_POST['save']) && !isset($_POST['reset']) && !isset($_POST['recount'])){
	...
	print('<tr><td>Количество рейтинга, необходимое для повышения до опытного пользователя:</td><td><input type="text" size="3" name="promote_rating" value="'.$CRON['promote_rating'].'"></td></tr>');
...
elseif (isset($_POST['save'])) {
	...
	foreach ($reqparametres as $param) {
		if (!isset($_POST[$param]) && (($param != 'rating_enabled') || ($param != 'delete_votes') || ($param != 'remote_trackers')))  { stdmsg($tracker_lang['error'],"Некоторые поля не заполнены ($param)",'error'); stdfoot(); die; }
