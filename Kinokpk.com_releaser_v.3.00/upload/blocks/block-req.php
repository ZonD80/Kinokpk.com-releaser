...
foreach ($reqarray as $requests) {
	...
	$content .= "<a href=\"requests.php?id=$requests[id]\"><b>$requests[request]</b></a>&nbsp;&nbsp;&nbsp;$done<br /><small> [комм:  $requests[comments], нуждающихся: $requests[hits]]<br /><a href=\"requests.php?action=vote&amp;voteid=$requests[id]\">Присоединиться к запросу</a></small><br />";
