...
if ($CURUSER) {
	...
	if (!$id) {$content .="<h1>Нет опросов!</h1>".((get_user_class() >= UC_ADMINISTARTOR)?"[<a href=\"pollsadmin.php?action=add\">Создать новый</a>]":""); } else {
		...
		<ul class="polls_title_q">
			<li class="colheadli"><h3 style="margin-top: 7px;margin-bottom:0;">'.$pquestion.'</h3>'.((get_user_class() >= UC_ADMINISTRATOR)?" <span style=\"margin-left: 335px;\">[<a href=\"pollsadmin.php?action=add\">Создать новый</a>] [<a href=\"pollsadmin.php?action=edit&amp;id=$id\">Редактировать</a>] [<a href=\"pollsadmin.php?action=delete&amp;id=$id\" onClick=\"return confirm('Вы уверены?');\">Удалить</a>]":"").'</span></li>
		</ul>';
		...
		foreach ($sidcount as $sidkey => $vsid){
			...
			if ($vsid == $voted)
			$content .="<ul><li class=\"polls_r\"><b>".$sidvals[$sidkey]." - ваш голос</b>";
  ...
  <input type=\"hidden\" name=\"type\" value=\"$ptype\" />".$sidvals[$sidkey];
		...
		$content .='<li style="text-align:center;">Всего голосов: '.$tvotes.', Комментариев: '.$comments.' [<a href="polloverview.php?id='.$id.'"><b>Подробнее</b></a>] [<a href="polloverview.php?id='.$id.'#comments"><b>Комментировать</b></a>] [<a href="pollsarchive.php"><b>Архив опросов</b></a>]</li></ul>'.($novote?'</form>':'');
