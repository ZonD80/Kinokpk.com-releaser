...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	sql_query("INSERT INTO bannedemails (added, addedby, comment, email) VALUES(".sqlesc(time()).", $CURUSER[id], ".sqlesc($comment).", ".sqlesc($email).")") or (mysql_errno() == 1062 ? stderr($tracker_lang['error'], "Этот e-mail уже забанен") : sqlerr(__FILE__, __LINE__));
...
else
{
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		print("<tr><td>".mkprettytime($arr[added])."</td><td align=left>$arr[email]</td><td align=left><a href=userdetails.php?id=$arr[addedby]>$a2[username]".
