...
<li nowrap=\"\" class=\"tab".($pend?'2':'1')."\"><a href=\"friends.php\"><span>Друзья</span></a></li>
<li nowrap=\"\" class=\"tab".($pend?'1':'2')."\"><a href=\"friends.php?pending\"><span>Ожидает подтверждения</span></a></li>
</ul></div>\n");
...
print("</select>\n");
print("<input class=\"button\" type=\"submit\" value=\"{$tracker_lang['go']}\" style=\"margin-top: -2px;\">\n");
print("</form>\n");
print("</div>\n");

$res = sql_query("SELECT SUM(1) FROM friends$querycount") or sqlerr(__FILE__, __LINE__);
$count = @mysql_result($res,0);
if (!$count) { stdmsg($tracker_lang['error'],$tracker_lang['no_friends'],'error'); stdfoot(); die(); }
$perpage = 50;
...
while ($arr = mysql_fetch_assoc($res)) {
	...
	print("</div>
			<div class=\"relgroups_name\">
				...
				<dt>Ник</dt><dd><a href=\"userdetails.php?id=$arr[friend]\"><b>".get_user_class_color($arr["class"], $arr["username"]).$gender.get_user_icons($arr).$country."</b></a></dd>
			 ...
			 <div id=\"input\"  class=\"relgroups_input\" >
				<ul  class=\"relgroups_input\">
					<li><a href=\"userdetails.php?id=$arr[friend]\">Просмотр</a></li>
					");
	if (!$pend)print ("<li><a href=\"friends.php?action=deny&amp;id=".$arr['id']."\">".$tracker_lang['delete_from_friends']."</a></li><li><a href=\"present.php?id=".$arr['friend']."\"><span>Подарок Другу</span><img src=\"pic/presents/present.gif\" title=\"Подарить подарок\" style=\"margin-top: -6px; border:none; width:12px; height:12px;\" /> </a></li><li>");
	elseif ($pend && !$arr['init']) print('<li>'.$tracker_lang['friend_pending'].'</li>');
	else print ("<li><a href=\"friends.php?action=confirm&amp;id=".$arr['id']."\">".$tracker_lang['confirm']."</a></li>
			<li><a href=\"friends.php?action=deny&amp;id=".$arr['id']."\">".$tracker_lang['delete_on_friends']."</a></li>
		  ");

	print("<li><a href=\"message.php?action=sendmessage&amp;receiver=".$arr['friend']."\">Личное Сообщение</a></li>");
	print ($arr['init']?"<li>{$tracker_lang['init']}</li>":'');
