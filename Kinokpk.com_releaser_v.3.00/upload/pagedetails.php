...
stderr($tracker_lang['error'], $tracker_lang['invalid_id']);
	...
	if (get_user_class()<$row['class'] && !$owned) stderr($tracker_lang['error'],$tracker_lang['access_denied']);
	...
	print("<tr><td class=\"colhead\" colspan=\"2\"><div style=\"float: left; width: auto;\">:: "."Просмотр страницы | <a href=\"pagebrowse.php\">К списку</a>"."</div><div align=\"right\"><a href=\"pagedetails.php?id=$id#comments\"><b>{$tracker_lang['add_comment']}</b></a></div></td></tr>");
	$url = "pageedit.php?id=" . $row["id"];
	...
	if ($childs) {
		...
		if (($cat['id'] != $child['id']) && in_array($child['id'],$cats)) $chsel[]="<a href=\"pagebrowse.php?cat={$child['id']}\">".makesafe($child['name'])."</a>";
	}
	tr ($tracker_lang['type'],get_cur_position_str($tree,$cat['id'],'pagebrowse').(is_array($chsel)?', '.implode(', ',$chsel):''),1);
	...
	$uprow = (isset($row["username"]) ? ("<a href=userdetails.php?id=" . $row["owner"] . ">" . get_user_class_color($row['uclass'],$row["username"]) . "</a>") : "<i>Аноним</i>");

	tr("Автор", ($row['owner']?$uprow.$spacer.ratearea($row['userrating'],$row['owner'],'users',$CURUSER['id']):$tracker_lang['from_system']), 1);
	tr('Индексируется в текстовых областях', ($row["indexed"]?$tracker_lang['yes']:$tracker_lang['no']));
	tr($tracker_lang['vote'],ratearea($row['ratingsum'],$id,'pages',(($row['owner']==$CURUSER['id'])?$id:0) ),1);
...
if (!$row['denycomments']) {
	...
	if (!$count) {
		...
		print("<div align=\"right\"><a href=pagedetails.php?id=$id#comments class=altlink_white>Добавить комментарий</a></div>");
		print("</td></tr><tr><td align=\"center\">");
		print("Комментариев нет. <a href=pagedetails.php?id=$id#comments>Желаете добавить?</a>");
		print("</td></tr></table><br />");

	}
	else {
		list($pagertop, $pagerbottom, $limit) = pager($limited, $count, "pagedetails.php?id=$id&", array('lastpagedefault' => 1));
		...
		while ($subrow = mysql_fetch_array($subres)) {
			...
			$subrow['link'] = "pagedetails.php?id=$id#comm{$subrow['id']}";
			$allrows[] = $subrow;
		}
		...
		print("<div align=\"right\"><a href=\"pagedetails.php?id=$id#comments\" class=\"altlink_white\">{$tracker_lang['add_comment']}</a></div>");
		print("</td></tr>");
	...
	print("<table style=\"margin-top: 2px;\" cellpadding=\"5\" width=\"100%\">");
	print("<tr><td class=colhead align=\"left\" colspan=\"2\">  <div id=\"comments\"></div><b>:: {$tracker_lang['add_comment']} к странице | ".is_i_notified($id,'pagecomments')."</b></td></tr>");
	print("<tr><td width=\"100%\" align=\"center\" >");
	//print("Ваше имя: ");
	//print("".$CURUSER['username']."<p>");
	print ( "<form name=comment method=\"post\" action=\"pagecomment.php?action=add\">" );
