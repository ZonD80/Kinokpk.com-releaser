...
if (!is_valid_id($rgid)) stderr($tracker_lang['error'],$tracker_lang['invalid_id']);
...
if ($count)
{
	...
	while(list($id, $userid, $added, $body, $subject,$comments) = mysql_fetch_array($resource))
	{
		...
		print("<tr><td style='background-color: #F9F9F9'>
            ...
            <div style='float:left;'><b>Размещено</b>: ".mkprettytime($added)." <b>Комментариев:</b> ".$comments." [<a href=\"rgnewsoverview.php?id=".$id."#comments\">Комментировать</a>]</div>");

		if ((get_user_class() >= UC_ADMINISTRATOR) || $I_OWNER)
		{
			print("<div style='float:right;'>
            <font class=\"small\">
            [<a class='altlink' href=\"rgnews.php?action=edit&amp;id=$rgid&amp;newsid=".$id."&returnto=".urlencode($_SERVER['PHP_SELF'])."\">Редактировать</a>]
            [<a class='altlink' onClick=\"return confirm('Удалить эту новость?')\" href=\"rgnews.php?action=delete&amp;id=$rgid&amp;newsid=".$id."&returnto=".urlencode($_SERVER['PHP_SELF'])."\">Удалить</a>]
            </font></div>");
		}
		print("</td></tr></table>");
