...
if ($action == "add")
{
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if ($last_pmrow[0]){
			...
			if ($CACHEARRAY['as_check_messages'] && ($last_pmrow[0]['msg'] == $text) && ($last_pmrow[1]['msg'] == $text) && ($last_pmrow[2]['msg'] == $text) && ($last_pmrow[3]['msg'] == $text)) {
				...
				foreach ($msgids as $key => $msgid){
					$msgview.= "\n<a href=relgroups.php?id={$torids[$key]}&viewcomm=$msgid#comm$msgid>Комментарий ID={$msgid}</a> от пользователя ".$CURUSER['username'];
				}
				...
				if (strpos($modcomment,"Maybe spammer in relgroupscomments") === false) {
					...
					while (list($admin) = mysql_fetch_array($arow)) {
						...
						$admin, '" . time() . "', 'Пользователь <a href=userdetails.php?id=".$CURUSER['id'].">".$CURUSER['username']."</a> может быть спамером, т.к. его 5 последних посланных комментариев к релиз группе полностью совпадают.$msgview', 'Сообщение о спаме!', 1)") or sqlerr(__FILE__, __LINE__);
					}
					...
					while (list($admin) = mysql_fetch_array($arow)) {
						sql_query("INSERT INTO messages (poster, sender, receiver, added, msg, subject, location) VALUES(0, 0,
						$admin, '" . time() . "', 'Пользователь <a href=userdetails.php?id=".$CURUSER['id'].">".$CURUSER['username']."</a> забанен системой за спам в комментариях к релиз-группам, его IP адрес (".$CURUSER['ip'].")', 'Сообщение о спаме [бан]!', 1)") or sqlerr(__FILE__, __LINE__);
						stderr("Поздравляем!","Вы успешно забанены системой за спам в комментариях к релиз-группам! Если вы не согласны с решением системы, <a href=\"contact.php\">подайте жалобу админам</a>.");
					}
				}
				stderr($tracker_lang['error'],"На нашем сайте стоит защита от спама, ваши 5 последних комментариев к релиз-группам совпадают. В отсылке комментария отказано. <b><u>ВНИМАНИЕ! ЕСЛИ ВЫ ЕЩЕ РАЗ ПОПЫТАЕТЕСЬ ОТПРАВИТЬ ИДЕНТИЧНОЕ СООБЩЕНИЕ, ВЫ БУДЕТЕ АВТОМАТИЧЕСКИ ЗАБЛОКИРОВАНЫ СИСТЕМОЙ!!!</u></b> <a href=\"javascript: history.go(-1)\">Назад</a>");
		...
		safe_redirect(" relgroups.php?id=$uid&viewcomm=$newid#comm$newid");
		die;
	}
}
elseif ($action == "quote")
{
	...
	print("<form method=\"post\" name=\"comment\" action=\"rgcomment.php?action=add\">\n");
...
elseif ($action == "edit")
{
	...
	print("<form method=\"post\" name=\"comment\" action=\"rgcomment.php?action=edit&amp;cid=$commentid\">\n");
	print("<input type=\"hidden\" name=\"returnto\" value=\"relgroups.php?id={$arr["uid"]}&amp;viewcomm=$commentid#comm$commentid\" />\n");
...
elseif ($action == "delete")
{
	...
	$returnto = "relgroups.php?id=$uid&viewcomm=$commentid#comm$commentid";
