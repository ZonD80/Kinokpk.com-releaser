...
if ($action == "add")
{
  if ($_SERVER["REQUEST_METHOD"] == "POST")
  {
    ...
    //   print_r($last_pmrow);
       if ($last_pmrow[0]){
        if (($as_timeout > round($last_pmrow[0]['seconds'])) && $as_timeout) {
          $seconds =  $as_timeout - round($last_pmrow[0]['seconds']);
        ...
        if ($as_check_messages && ($last_pmrow[0]['msg'] == $text) && ($last_pmrow[1]['msg'] == $text) && ($last_pmrow[2]['msg'] == $text) && ($last_pmrow[3]['msg'] == $text)) {
            $msgview='';
