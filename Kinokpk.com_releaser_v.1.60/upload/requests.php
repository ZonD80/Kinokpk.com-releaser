...
if ($action == 'new') {
        ...
        $id = mysql_insert_id();
                    ...
                    mysql_query("UPDATE users SET bonus=bonus+5 WHERE id =".$CURUSER['id']);
...
if ($action=='reset')
{
  if (!is_numeric($_GET['requestid'])) die ("Access denied: Wrong request ID");
...
print("<tr><td align=left>Опции</td><td width=50% align=left><a href=viewrequests.php?delreq[]=$id>".$tracker_lang['delete']."</a> <b>|</b> <a href=requests.php?action=reset&requestid=$id>Сбросить выполнение</a>  <b>|</b>  <a href=requests.php?action=edit&id=$id>".$tracker_lang['edit']."</a></center></td></tr>");
