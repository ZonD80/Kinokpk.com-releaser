...
if ($_GET[offerid])
{
        ...
        $res2 = sql_query("SELECT COUNT(offervotes.id) FROM offervotes INNER JOIN users ON offervotes.userid = users.id INNER JOIN offers ON offervotes.offerid = offers.id WHERE offervotes.offerid = ".sqlesc($offerid)) or sqlerr(__FILE__,__LINE__);
        $row = mysql_fetch_array($res2);
        $count = $row[0];
        $perpage = 50;
        list($pagertop, $pagerbottom, $limit) = pager($perpage, $count, $_SERVER["PHP_SELF"] ."?" );

        $res = sql_query("SELECT users.id as userid, users.username, users.downloaded, users.uploaded, users.class, offers.id AS offerid, offers.name FROM offervotes INNER JOIN users ON offervotes.userid = users.id INNER JOIN offers ON offervotes.offerid = offers.id WHERE offervotes.offerid =$offerid $limit") or sqlerr(__FILE__,__LINE__);
        stdhead("Голосовавшие");
        $res2 = sql_query("SELECT name FROM offers WHERE id=$offerid");
        $arr2 = mysql_fetch_assoc($res2);
        print("<h1>Голосовавшие для <a href=offers.php?id=$offerid><b>$arr2[name]</b></a></h1>");
        print("<p>Голосовать за это <a href=offers.php?action=vote&voteid=$offerid><b>предложение</b></a></p>");
