...
<tr>
<td valign="middle" class=rowhead>Уровень<br/>предупреждения:</td>
  <td<?=$_GET['as1']?$highlight:""?>><select name="as1">
    <?
    $options = array("(Любой)" ,"0 звезд","1 звезда","2 звезды","3 звезды","4 звезды","5 звезд");
    for ($i = 0; $i < count($options); $i++){
...
if (count($_GET) > 0 && !$_GET['h'])
{
  ...
  $select_is = "u.id, u.username, u.email, u.status, u.added, u.last_access, u.ip,
      u.class, u.uploaded, u.downloaded, u.donor, u.modcomment, u.enabled, u.warned, u.num_warned";
