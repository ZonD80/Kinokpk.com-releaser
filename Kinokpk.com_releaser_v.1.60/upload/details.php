...
stdmsg("Ошибка", "Вы не можете подать жалобу на собственную раздачу");
                ...
                if ($row["image1"] != "" OR $row["image2"] != "") {
                  if ($row["image1"] != "")
                    $img1 = "<a href=\"viewimage.php?image=$row[image1]\"><img border='0' alt='Постер для ".$row["name"]." (кликните для просмотра полного изображения)' src=\"thumbnail.php?image=$row[image1]&for=details\" /></a>";
                  if ($row["image2"] != "")
                    $img2 = "<a href=\"viewimage.php?image=$row[image2]\"><img border='0' alt='Кадр из ".$row["name"]." (кликните для просмотра полного изображения)' src=\"thumbnail.php?image=$row[image2]&for=details\" /></a>";
...
while ($did = mysql_fetch_array($detid))  {
  if (($did['hide'] == 'yes') && !$CURUSER) tr($did['name'],"Хотите посмотреть? <a href=\"login.php?returnto=".urlencode(basename($_SERVER["REQUEST_URI"]))."\">Войдите</a> или <a href=\"signup.php?returnto=".urlencode(basename($_SERVER["REQUEST_URI"]))."\">Зарегестрируйтесь</a> на $DEFAULTBASEURL",1); else {
  if ($did['value'] != '') tr($did['name'],format_comment($did['value']),1);
  }
}
tr("Релиз на форуме $FORUMNAME","<a href=\"$FORUMURL/index.php?showtopic=".$row['topic_id']."\">$FORUMURL/index.php?showtopic=".$row['topic_id']."</a>",1);
                 tr("Смотрели?","<form method=\"post\" action=\"takesnatched.php\"><input type=\"hidden\" name=\"id\" value=\"".$row['id']."\" /><input type=\"submit\" value=\"Смотрел(а), но не качал(а) с $DEFAULTBASEURL\" /></form>",1,1);
                 if (!empty($row['online']))
                // tr("Смотреть онлайн<br />","<form method=\"post\" action=\"online/onlinevideo.php\"><input type=\"hidden\" name=\"onlinevideo\" value=\"".$row['online']."\"><input type=\"submit\" value=\"Смотреть онлайн на $DEFAULTBASEURL\" /></form> <b>ТОЛЬКО ДЛЯ КПК НА WINDOWS MOBILE 5.0 ВЫШЕ! ФОРМАТ WMV</b>",1,1);
                if ($row["visible"] == "no")
                ...
                $sql = sql_query("SELECT filename FROM torrents WHERE id = ".$id);
          $nf = mysql_result($sql,0);
          if ($nf == 'nofile') {} else
...
$torrentid = (int) $_GET["id"];
				...
				if ($CURUSER["id"] != $row["owner"])  
                    tr("Пожаловаться", "<form method=\"post\" action=\"report.php?id=".$row['id']."\">&nbsp;<input name=motive cols=40 value=\"Причина\">&nbsp;<input type=\"submit\" value=\"Отправить\" /><input type=\"hidden\" name=\"torrentid\" value=\"$torrentid\"></form>", 1);  
...
if (!$count) {
  ...
  print("<div style=\"float: left; width: auto;\" align=\"left\"> :: Список комментариев $DEFAULTBASEURL</div>");
...
print("<table class=main cellspacing=\"0\" cellPadding=\"5\" width=\"100%\" >");
         print("<tr><td class=\"colhead\" align=\"center\" >");
         print("<div style=\"float: left; width: auto;\" align=\"left\"> :: Список комментариев форума $FORUMNAME</div>");
         //print("<div align=\"right\"><a href=\"$FORUMURL/index.php?act=post&do=reply_post&f=".$forumid."&t=".$topicid."\" class=altlink_white>Добавить комментарий</a></div>");
         print("</td></tr>");
