...
$res = sql_query("SELECT id, passhash, secret, enabled, dis_reason, status FROM users WHERE username = " . sqlesc($username));
...
$ipbuser = mysql_query("SELECT id, email, bday_day, bday_month, bday_year FROM ".$fprefix."members WHERE name=".sqlesc($username));
...
$pwd = mysql_query("SELECT converge_pass_hash,  converge_pass_salt FROM ".$fprefix."members_converge WHERE converge_email='".$ipbuser['email']."'");
...
$ipbusere = mysql_query("SELECT aim_name, icq_number, website, yahoo, msnname, avatar_location, avatar_type FROM ".$fprefix."member_extra WHERE id=".$ipbuser['id']);
$ipbusere = mysql_fetch_array($ipbusere);

$forumgender = mysql_query("SELECT pp_gender FROM ".$fprefix."profile_portal WHERE pp_member_id=".$ipbuser['id']);
...
$ret = sql_query("INSERT INTO users (username, passhash, secret, editsecret, gender, country, icq, msn, aim, yahoo, website, email, status, ". (!$users?"class, ":"") ."added, birthday, invitedby, invitedroot, avatar) VALUES (" .
		...
		", ". (!$users?UC_SYSOP.", ":""). "'". get_date_time() ."', '$birthday', 0, 0, '$uavatar')");// or sqlerr(__FILE__, __LINE__);
