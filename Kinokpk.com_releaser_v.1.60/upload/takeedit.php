...
if (!is_numeric($_GET['id']) && ($_SERVER['REQUEST_METHOD'] != 'POST')) { die ("Access denied: Wrong ID");
$id = 0 + $_GET["id"];
} else $id = 0+$_POST['id'];
...
if ($_POST['nofile'] == 'yes') {
 ...
 $wastor = mysql_query("SELECT filename FROM torrents WHERE id =".$id);
 $wastor = mysql_result($wastor,0);

 if ($wastor != 'nofile') {
   mysql_query("DELETE FROM files WHERE torrent = ".$id);
   mysql_query("DELETE FROM peers WHERE torrent = ".$id);
   mysql_query("DELETE FROM snatched WHERE torrent = ".$id);
   mysql_query("UPDATE torrents SET leechers = 0, seeders = 0");
...
sql_query("UPDATE torrents SET " . join(",", $updateset) . " WHERE id = $id");
 ...
 foreach($_POST['val'] as $descrid => $content) {
     mysql_query("UPDATE descr_torrents SET value = ".sqlesc($content)." WHERE id =".$descrid);
 }


/// IPB INTEGRATION ///// EDIT WIKI CONTAINER ////////////
$image = mysql_query("SELECT image1 FROM torrents WHERE id = $id");
...
$image = "<div align=\"center\"><img src=\"$DEFAULTBASEURL/pic/noimage.gif\" border=\"0\" class=\"linked-image\" /></div><br />";
  ...
  if (!empty($_POST['topic'])) {
    ...
    if (is_numeric($topicid)) {
      $topicid = 0 + $topicid;
      mysql_query("UPDATE torrents SET topic_id =".$topicid." WHERE id =".$id);
      $topicedit = 1;
    } else die("Неверный ID темы");
    }  else {
$topicid = mysql_query("SELECT topic_id FROM torrents WHERE id = $id");
...
if ($topicid <> 0) {
$forumdesc = $image;
$forumdesc .= "<table width=\"100%\"><tr><td><b>Название:</b></td><td>$name</td></tr>";
$detid = mysql_query("SELECT descr_torrents.value, descr_details.name, descr_details.input FROM descr_torrents LEFT JOIN descr_details ON descr_details.id = descr_torrents.typeid WHERE descr_torrents.torrent = ".$id." AND descr_torrents.value <> '' ORDER BY descr_details.sort ASC");
...
$isnofilesize = mysql_query("SELECT filename,size FROM torrents WHERE id = $id");
$isnofilesize = mysql_fetch_array($isnofilesize);
$topicfooter = "<tr><td width=\"150px\"><b>Размер файла:</b></td><td>".round($isnofilesize['size']/1024/1024)." МБ</td></tr></table>";


if ($isnofilesize['filename'] != 'nofile')
  $topicfooter .= "<br /><br />Torrent:<br />Нажмите на ссылку ниже"; else $topicfooter .= '';

$topicfooter .= "<br /><br /><br /><div align=\"center\">[<span style=\"color:#FF0000\"><a href=\"$DEFAULTBASEURL/details.php?id=".$id."&hit=1\">Этот релиз на $DEFAULTBASEURL</a></span>]</div>";
...
mysql_query("UPDATE ".$fprefix."posts SET wiki = '".$forumdesc."' WHERE pid=".$postid);
...
if ($topicedit) {
  ...
  $topic = mysql_query("UPDATE ".$fprefix."topics SET title = '".$topicname."'".$dsql." WHERE tid =".$topicid);
