		...
		// move event
			...
			_opener.bind('mousedown', function(e){
				...
				$(document).bind('mousemove', function(ev){
					...
					if (_changeX) { _def = _xDef; _change = _x; }
					if (_changeY) { _def = _yDef; _change = _y; }
					
					_dif = _defParam + (_change - _def)*_revers;
					
					if (_setParam < _defParam) {
						if (_dif < _setParam) {_dif = _setParam;}
						if (_dif >= _defParam) {_dif = _defParam; _obj.removeClass('blockChange'); _obj.addClass("blockDefault");}
						else {_obj.addClass('blockChange'); _obj.removeClass("blockDefault");}
					} else {
						if (_dif > _setParam) {_dif = _setParam;}
						if (_dif <= _defParam) {_dif = _defParam; _obj.removeClass('blockChange'); _obj.addClass("blockDefault");}
						else {_obj.addClass('blockChange'); _obj.removeClass("blockDefault");}
