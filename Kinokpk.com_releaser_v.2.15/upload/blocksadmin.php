...
$allowed_modules = array(
  ...
  "users" => "Пользователи",
  "topten" => "Топ 10",
  "formatss" => "Форматы файлов",
  "formats" => "Выбор форматов файлов",
...
function BlocksAdmin() {
	...
	echo "</td></tr></table><center>[ <a href=\"blocksadmin.php?op=BlocksFixweight\">Зафиксировать позицию и положение блоков</a> ]</center>";
...
function BlocksNew() {
	...
	while ($file = readdir($handle)) {
		if (preg_match("/^block\-(.+)\.php/", $file, $matches)) {
			...
			if (mysql_num_rows(sql_query("SELECT * FROM orbital_blocks WHERE blockfile='$file'")) == 0) echo "<option value=\"$file\">$found</option>\n";
		}
	}
	closedir($handle);
	echo "</select></td></tr>"
...
function BlocksChange($bid, $ok=0) {
	...
	if (($ok) || ($active == 0)) {
		...
		echo "[ <a href=\"blocksadmin.php?op=BlocksChange&bid=$bid&ok=1\">Да</a> | <a href=\"blocksadmin.php\">Нет</a> ]</center>";
	}
}
...
switch($op) {
	...
	case "BlocksChange":
	BlocksChange($_GET['bid'], $_GET['ok'], $_GET['de']);
	break;
