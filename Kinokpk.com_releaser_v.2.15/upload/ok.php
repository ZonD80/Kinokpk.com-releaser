...
if ($type == "signup" && mkglobal("email")) {
	...
	stdhead($tracker_lang['signup_successful']);
        stdmsg($tracker_lang['signup_successful'],($CACHEARRAY['use_email_act'] ? sprintf($tracker_lang['confirmation_mail_sent'], htmlspecialchars($email)) : sprintf($tracker_lang['thanks_for_registering'], $CACHEARRAY['sitename'])));
	stdfoot();
}
elseif ($type == "sysop") {
		stdhead($tracker_lang['sysop_activated']);
	if (isset($CURUSER))
		stdmsg($tracker_lang['sysop_activated'],sprintf($tracker_lang['sysop_account_activated'], $CACHEARRAY['defaultbaseurl']));
	else
...
elseif ($type == "confirm") {
	if (isset($CURUSER)) {
		...
		print("<p>Презжде чем начать использовать {$CACHEARRAY['sitename']} мы рекомендуем вам прочитать <a href=\"rules.php\"><b>правила</b></a> и <a href=\"faq.php\"><b>ЧаВо</b></a>.</p>\n");
		stdfoot();
	}
	else {
