...
if ($count)
{
 ...
 $addparam = $pagerlink;
        ...
        "IF(torrents.numratings < {$CACHEARRAY['minvotes']}, NULL, ROUND(torrents.ratingsum / torrents.numratings, 1)) AS rating, categories.name AS cat_name, categories.image AS cat_pic, users.username, users.class, ratings.rating AS rrating, ratings.added AS radded FROM torrents LEFT JOIN categories ON category = categories.id LEFT JOIN users ON torrents.owner = users.id LEFT JOIN ratings ON (torrents.id = ratings.torrent ".($CURUSER?"AND ratings.user = ".$CURUSER['id']:"").") $where $orderby $limit";
...
<tr><td class="colhead" align="center" colspan="12">Список релизов</td></tr>
<tr><td colspan="12">
...
foreach ($cats as $cat)
{
      ...
      $tagarray = substr($tagarray,2);
        print("<tr><td width=\"30%\"><input name=\"c$cat[id]\" type=\"checkbox\" " . (in_array($cat[id],$wherecatina) ? "checked " : "") . "value=\"1\"><a class=\"catlink\" href=\"browse.php?cat=$cat[id]\">" . htmlspecialchars($cat[name]) . "</a></td><td><i>Список тегов/жанров для этой категории</i>:<br/>$tagarray</td></tr>\n");
}
...
<option value="5"<? print($_GET["incldead"] == 5 ? " selected" : ""); ?>>Без торрентов</option>
</select>
<select name="cat">
...
<form action="http://www.google.com/cse" id="cse-search-box">
    ...
    <input name="sa" value="Поиск Google!" type="submit">
</form>
<!-- / Google Search -->
</center>
