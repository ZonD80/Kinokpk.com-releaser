...
/*	$avatar = $_POST["avatar"];
	...
	if ($avatar) {
		...
		if(!(list($width, $height) = @getimagesize($avatar)))
						stderr($tracker_lang['error'], $tracker_lang['avatar_adress_invalid']);
		if ($width > $CACHEARRAY['avatar_max_width'] || $height > $CACHEARRAY['avatar_max_height'])
						stderr($tracker_lang['error'], sprintf($tracker_lang['avatar_is_too_big'], $CACHEARRAY['avatar_max_width'], $CACHEARRAY['avatar_max_height']));
	}
// Check remote avatar size */
	$resetb = $_POST["resetb"];
