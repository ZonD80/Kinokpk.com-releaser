...
<legend>Правила релизера <?=$CACHEARRAY['sitename']?></legend>
<form method="post" action="<?=$PHP_SELF?>">
<table cellpadding="4" cellspacing="0" border="0" style="width:100%" class="tableinborder">
<tr>
<td class="tablea">Для продолжения регистрации, Вы должны согласиться со следующими правилами:<br />
<font color="red">ВНИМАНИЕ! ВЫ ТАКЖЕ ЗАРЕГЕСТРИРУЕТЕСЬ НА ФОРУМЕ <?=$CACHEARRAY['forumname']?>!!!<br />
Если уже у вас есть аккаунт на форуме <?=$CACHEARRAY['forumname']?>, то войдите под своим логином и паролем.</font></td></tr>
<tr>
...
<p><strong>Правила <?=$CACHEARRAY['sitename']?> и <u><a target="_blank" href="<?=$CACHEARRAY['forumurl']?>/index.php?act=boardrules">форума <?=$CACHEARRAY['forumname']?> (перейти)</a></u></strong></p>
...
Если вы передумали регистрироваться, нажмите <a href="<?=$CACHEARRAY['defaultbaseurl'];?>">здесь</a>, чтобы вернуться на главную страницу.</p>
<p>Хотя модераторы и администраторы, обслуживающие <?=$CACHEARRAY['sitename'];?>, стараются удалять все оскорбительные и некорректные
...
<input type="hidden" name="do" value="register">
  <strong>Я согласен исполнять установленные правила, посещая <?=$CACHEARRAY['sitename'];?>.</strong>
</label>
</div>
</td></tr>
</table>
