...
$count = get_row_count("polls");
  ...
  $addparam = $_SERVER['QUERY_STRING'];
   ...
   if ($addparam != "")
    ...
    $addparam = $pagerlink;
        list($pagertop, $pagerbottom, $limit) = pager(5, $count, "polloverview.php?" . $addparam);
  ...
  while (list($id) = mysql_fetch_array($pollsrow)) {
   ...
   while ($pollarray = mysql_fetch_array($poll)) {
    ...
    print('<tr><td><table width="100%" border="1"><tr><td>Опрос № '.$id.'</td><td>Открыт: '.get_date_time($pstart).(!is_null($pexp)?", заканчивается ".get_date_time($pexp):"").'</td></tr><tr><td align="center" class="colhead" colspan="2">'.$pquestion.'</b>'.((get_user_class() >= UC_ADMINISTRATOR)?" [<a href=\"pollsadmin.php?action=edit&id=$id\">Редактировать</a>][<a onClick=\"return confirm('Вы уверены?')\" href=\"pollsadmin.php?action=delete&id=$id\">Удалить</a>]":"").'</td></tr>');
...
//      print($vote['vid'].$vote['username'].$vote['userclass'].$vote['userid'].",");
 ...
 foreach ($sidcount as $sidkey => $vsid){
  ...
  print("</td><td><img src=\"./themes/$ss_uri/images/bar_left.gif\"><img src=\"./themes/$ss_uri/images/bar.gif\" height=\"12\" width=\"".round($percentpervote*$votecount[$vsid])."%\"><img src=\"./themes/$ss_uri/images/bar_right.gif\">$percent%, голосов: ".$votecount[$vsid]."<br/>".((!$usercode[$vsid])?"Никто не голосовал":$usercode[$vsid])."</td></tr>");
}
